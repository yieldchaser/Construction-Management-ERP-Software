...
1. Four backend tests are RED at HEAD — ALL CLASSIFIED category (a) "stale test/bookkeeping" (verified by reading the live code, NOT by trusting the landed fix blindly). None is category (b) "gate edited to mirror drifted code":
   - `test_prompt7_features.py::test_import_boq_preserves_cost_code` — R2-334's finding REQUIRED the Cost Code Library gate the test predates; fix = seed LibraryCostCode rows for 1.1/1.2 in the fixture. (a)
   - `test_pin_R2_067` — R2-233's long-form prescribed the approved-only/non-cancelled filter; seeds lack approval_flag='approved'. Fix fixture, keep formula assertion. (a)
   - `test_pin_R2_134` — live three_way.py still has MATCH_TOLERANCE_PCT=0.01 (:14) and the max(min, pct*po_amount) gate (:145); R2-241 only split it into a named `tolerance` var so the pinned inline substring vanished. Update pin substring. (a)
   - `test_pin_R2_405` — live settings.py still reads mobile (`phone=u.mobile` :620, `user.mobile if user else None` :674); R2-390 removed the `.first()` chain shape the pin also asserted. Update pin to assert mobile reads / absence of phone-column reads. (a)
2. Central pin collection for Session 33's fixes is PENDING (pin suggestions collected in SESSION_LOG).
3. R2-280 + R2-519 (H-calculators) remain TODO — subagent dispatches kept failing; audit fix sketches are at reg L12310 / L26418.
4. Process notes from the founder's independent verification pass are ADOPTED: constraints/indexes must ship supabase migrations (honored: R2-594, R2-334, R2-377); no overclaiming pin strength; defect-class sweeps before closing; every disclosed sibling gets a filed id. `docs/AUDIT_REGRESSION_SUITE.md` restored from orphan branch 27fab37 (R2-725) so RC-citations in FIX_VERIFIED rows resolve; ids never defined anywhere remain honestly undefined.
5. Verification-pass findings R2-701..R2-725 live in docs/VERIFICATION_NEW_FINDINGS.md on branch claude/siteflow-live-verification-dba0f1. Being fixed now: R2-721 (statutory report_type unvalidated), R2-722 (auth demo-company seeding + OTP demo defaults), R2-723 (8 bill aggregations miss Cancelled exclusion).

...
| R2-038 | LOW | W81 | `frontend/src/app/c/[company_id]/analytics/page.tsx` | — | FIXED | <insert-commit-hash> | reg L1662; the analytics page now uses the shared `fmtINR` formatter (from `@/lib/siteflow`) instead of a local `Rs `-prefixed `formatCurrency` that ignored `currency_decimal_places`. All 6 `Rs ` literals removed (Budget Variance card, Spend vs budget hint, Project Budget table). The local `formatCurrency` is now a 1-line wrapper around `fmtINR`. NOTE: `fmtINR` defaults to 0 decimal places — analytics page does not have the company settings in scope so the configured decimals are still deferred (same pattern as the rest of the codebase). Blast-radius 1 file, +5/-4 lines. No test added.
...
| R2-044 | MEDIUM | W07 | `billing.py` | — | TODO | | reg L1919 |

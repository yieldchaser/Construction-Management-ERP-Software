...
| D-023 | MEDIUM | Subscription billing (Razorpay/Stripe subscriptions for company plans). Multi-tier SaaS metering, self-serve plan upgrades, and recurring billing. | Founder-owned; deliberately deferred until customer pilot finishes. Self-serve plan upgrades are not on critical path for MVP. | billing.py / settings.py |

...
| D-019 | MEDIUM | `uq_bills_po_id` / `uq_equipment_company_id_code` may be skipped on production | Both migrations skip with a NOTICE when existing data would violate the constraint, rather than deleting rows. | Production DDL + any purge are founder-owned (Part D, D1). Until a keep-earliest purge runs, the constraint is not created and the guarantee is not enforced in production. Re-run the migration after the purge. |

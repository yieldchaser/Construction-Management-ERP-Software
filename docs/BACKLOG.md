...
| D-021 | ~~HIGH — FOUNDER~~ | ~~**GitHub Actions is billing-blocked**~~, so the `Apply Supabase Migrations` workflow and `Keep Alive Backend` never start. Run 33297710520: *"The job was not started because recent account payments have failed or your spending limit needs to be increased."* | **CLOSED 2026-08-31.** The repository was made public, so Actions now run free. Workflow run 33330984105 () succeeded in 38s and applied the 3 pending migrations: `20260829_000003_staff_employees_uan`, `20260829_000004_crm_lead_priority_normalized`, `20260830_000001_chat_group_member_last_read_at` (3/56 pending, ledger updated). | Fix billing, then re-run the workflow (`workflow_dispatch`). Both pending migrations are idempotent, so a re-run is safe. |

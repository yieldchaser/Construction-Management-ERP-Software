...
| R2-336 | MEDIUM | billing.py / settings.py | Subscription billing (Razorpay/Stripe subscriptions for company plans). Multi-tier SaaS metering, self-serve plan upgrades, and recurring billing. | D-023 | Founder-owned; deliberately deferred until customer pilot finishes. Self-serve plan upgrades are not on critical path for MVP. |

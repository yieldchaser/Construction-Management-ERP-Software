...
| R2-027 | CRITICAL | FIXED | `97f4eb4` | `face_recognition.py` | yes | yes | text-pin | 1 | CONFIRMED | E0 LIVE: face_recognition_logs.created_at exists in Supabase as timestamp with time zone, so the Sentry-proven AttributeError 500 cannot recur. (My first query said ABSENT - I had used the singular table name. The table is face_recognition_logs.) |
...
| R2-086 | CRITICAL | FIXED | `97f4eb4` | `face_recognition.py` | yes | yes | text-pin | 1 | CONFIRMED | E0 LIVE: same column, same query - present. Rides on R2-027. |
...
| R2-196 | HIGH | FIXED | `aaa5041` | `auth.py` | yes | yes | text-pin | 1 | CONFIRMED | E0 LIVE: the revoked_tokens TABLE exists and users.tokens_revoked_at exists as timestamp with time zone, so the token-revocation migration landed. |
| R2-202 | HIGH | FIXED | `42d2c9a` | `planning.py` | yes | yes | text-pin | 1 | UNVERIFIED | E0 passes - column live. Behaviour not yet exercised. |
| R2-204 | HIGH | FIXED | `5b37186` | `quality.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: NCR review stamps reviewed_by from current_user (:389) and close stamps closed_by (:415). |
...
| R2-029 | MEDIUM | FIXED | `ce0e154` | `zoho_books.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: _search_vendor takes contact_type with a 'vendor' default and applies it only when set, so the duplicate search can run unfiltered across contact types. |
| R2-031 | MEDIUM | FIXED | `c962290` | `planning.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: update_task derives status from progress when status is not supplied, and progress from status when progress is not - each guarded so an explicit value always wins. |
| R2-037 | MEDIUM | FIXED | `df91126` | `analytics.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: wastage is gated on has_consumption - the quantity is zero and the percentage is None when nothing has been consumed (analytics.py:344-346), so un-issued stock no longer reports 100%. |
...
| R2-048 | MEDIUM | FIXED | `e8ad9cd` | `helpContent.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E1: the Help page renders a real Modules directory from HELP_MODULE_LINKS. |
| R2-054 | MEDIUM | FIX_VERIFIED | `6e43ff0` | `finance.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: the PR number generator loops on a candidate and re-queries PaymentRequest.request_no == candidate until free (finance.py:1119-1125), so a deletion no longer reissues a live number. |
...
| R2-082 | MEDIUM | FIXED | `248c809` | `frontend/src/app/c/[company_id]/analytics/page.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E1: burn-rate and labour KPIs render an em-dash for no-data rather than a fabricated zero, and subcontractor names resolve through library_party. |
...
| R2-098 | MEDIUM | FIXED | `6e43ff0` | `UNMAPPED` | yes | yes | text-pin | 1 | CONFIRMED | E1: the party PID generator uses the same candidate loop against LibraryParty.party_id_custom (library.py:145). |
| R2-102 | MEDIUM | FIXED | `8a0def3` | `backend/app/models.py` | yes | yes | text-pin | 1 | CONFIRMED | E1 + E0: the ONS- to SF- template change is a Python-side default and it is applied at the shipped sites. Note for the record: the voucher_number_template column has NO database default in production, so the value depends entirely on the ORM supplying it - correct today, but a non-ORM insert would leave it null. |
...
| R2-107 | MEDIUM | FIXED | `7ffa1c9` | `d/attendance/page.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E1: no hardcoded UI date defaults survive - the only 2026-06-30 / 2026-07-04 strings left in frontend/src are blog publishDate values in CMS content, which are legitimate. |
...
| R2-124 | MEDIUM | FIXED | `89839c9` | `frontend/src/app/c/[company_id]/d/equipment/page.tsx` | yes | yes | parse_fail | 1 | CONFIRMED | E1: both equipment pages render honest empty states, with a CTA on Fleet. |
...
| R2-136 | MEDIUM | FIXED | `048f72f` | `planning.py` | yes | yes | text-pin | 1 | CONFIRMED | E1, complete on both schemas: the predecessor link type carries PREDECESSOR_LINK_TYPE_PATTERN (:72) and milestone type/status carry their patterns on create (:92-93) AND update (:99-100). The row labels itself PARTIAL for the instances outside planning.py. |
| R2-144 | MEDIUM | FIXED | `b2ddf1f` | `page.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E1: media_url and voice_note_url are gone from the chat router's client contract. |
...
| R2-147 | MEDIUM | FIXED | `b8c36c4` | `chat.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: the message poll takes a since_id cursor and resolves it to an anchor timestamp before filtering (chat.py:191-209). |
...
| R2-154 | MEDIUM | FIXED | `bd41ec7` | `budget.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: committed counts POs whose status is in (sent, partial, received) and work orders that are not cancelled (budget.py:82, :94) - so draft and cancelled documents no longer inflate commitment. |
...
| R2-206 | MEDIUM | FIXED | `83c32c2` | `production.py` | yes | yes | text-pin | 1 | NOT_IN_PROD | E0 LIVE, and this one is a real miss. The row claims reported_by was converted to a UUID FK by migration 20260816_000005. In Supabase it is still `character varying`, there is NO foreign key on it, and 2 of the 3 material_wastage rows hold non-UUID free text. The boot sync adds columns but cannot change a column's TYPE, so this migration never ran. Raised as R2-730. |
...
| R2-217 | MEDIUM | FIXED | `f38215e` | `d/drawings/page.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E0 LIVE: drawing_pins.resolved exists in Supabase as boolean, so migration 20260816_000006 did land. Its disclosed sibling (handleAddPin local-only on failure) sits in R2-717. |
...
| R2-255 | MEDIUM | FIXED | `bd1c9f7` | `planning.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: duration_days is Field(ge=0) on BOTH the create and the update schema (planning.py:53, :63). |
| R2-256 | MEDIUM | FIXED | `d879c01` | `safety.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: close_incident stamps closed_by from the session (safety.py:157) and the column is returned in both read paths. |
| R2-261 | MEDIUM | FIXED | `7b868ea` | `team_schedule.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: create_dpr returns 409 on a second DPR for the same project and date. Register attributes this to team_schedule.py; the fix is in dpr.py, which the note says. |
...
| R2-295 | MEDIUM | FIXED | `fed144d` | `rate_limit.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: the limiter takes a shared storage_uri when RATE_LIMIT_STORAGE_URI is set and falls back to in-process storage when it is not (rate_limit.py:11-12) - config-gated, as claimed. |
...
| R2-370 | MEDIUM | FIXED | `3e6ecb0` | `billing.py` | yes | yes | text-pin | 1 | CONFIRMED | E0 LIVE: bills.cancelled_at exists as timestamp with time zone, so migration 20260816_000003 landed. This row also carries the R2-232 content onto this lineage, which is why R2-232 verified cleanly. |
| R2-373 | MEDIUM | FIXED | `b298269` | `models.py` | yes | yes | text-pin | 1 | CONFIRMED | E0 LIVE: material_indents.approved_by exists as uuid, so migration 20260816_000001 landed. |
...
| R2-379 | MEDIUM | FIXED | `b2ddf1f` | `billing.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: Advance Recovery sums the party+project advance cumulatively and rejects a draw beyond it (billing.py:661-669). |
...
| R2-411 | MEDIUM | FIXED | `5847922` | `models.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: the Tally XML builder emits an ALTERID element (tally_xml.py:57), which is the create-if-absent ledger-master behaviour the finding asked for. |
| R2-420 | MEDIUM | FIXED | `e069dfd` | `finance/page.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E1: the party balance renders through Math.abs with a direction chip and TransactionRow carries project_name. |
| R2-428 | MEDIUM | FIXED | `cd01b15` | `d/finance/page.tsx` | yes | yes | EVIDENCE_CLOSE | 1 | UNVERIFIED |  |
| R2-436 | MEDIUM | FIXED | `8025709` | `d/mom/page.tsx` | yes | yes | FAKE? | 1 | CONFIRMED | E1: `form.created_by` is gone from both MOM pages. This is the row whose pin gatecheck flagged as partially vacuous - the fix is sound, the pin's third assertion is not (R2-717 family). |
...
| R2-446 | MEDIUM | FIXED | `99f1442` | `mom/page.tsx` | yes | yes | parse_fail | 1 | CONFIRMED | E1: MOM_STATUSES includes Draft. |
...
| R2-461 | MEDIUM | FIXED | `94988a2` | `planning.py` | yes | yes | text-pin | 1 | CONFIRMED | E1, complete across all three paths the note claims - the inclusive `duration - 1` appears in propagate (:224), create (:451) and update (:527). |
...
| R2-508 | MEDIUM | FIXED | `deb3a9a` | `UNMAPPED` | yes | yes | text-pin | 1 | CONFIRMED | E1: ltif_basis is a parameter defaulting to 200000 with the OSHA convention documented at safety.py:175-183. |
...
| R2-566 | MEDIUM | FIXED | `43fe151` | `planning.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: TaskCreateRequest.status defaults to not_started and the constructor uses it, so a client omitting status gets the declared default rather than an empty string. |
...
| R2-108 | MEDIUM | FIXED | `a2a6566` | `UNMAPPED` | no | no | — | 2 | CONFIRMED | E1: employee creation compares trimmed-lowercase names against the existing list and asks for confirmation before creating a second one, rather than silently duplicating. |
...
| R2-121 | MEDIUM | FIXED | `25f30db` | `billing.py` | no | no | — | 3 | CONFIRMED | E1: the Subcon page no longer renders terminal empty states in place of real data. |
...
| R2-104 | LOW | FIXED | `a99e206` | `finance/page.tsx` | yes | yes | text-pin | 1 | CONFIRMED | E1: the Tally panel's last-export and last-marked summaries are real state (finance.py:266-267) rendered with an honest 'Not yet' when unset. |
| R2-120 | LOW | FIXED | `06cde63` | `google_drive.py` | yes | yes | text-pin | 1 | CONFIRMED | E1: the Integrations page lists the connectors as the note describes. |
...
| R2-023 | LOW | FIXED | `6ef2cc8` | `analytics/page.tsx` | no | no | — | 2 | CONFIRMED | E1: no PHASE build label survives anywhere in frontend/src. |

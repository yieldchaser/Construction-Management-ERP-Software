...
export default function SubconScorecardsPage() {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen text-muted text-xs">Loading scorecards...</div>;
    ...
    <div className="flex h-screen bg-background text-foreground overflow-hidden font-sans">
      <aside className="w-56 border-r border-border-custom bg-card flex flex-col shrink-0">
        ...
        <nav className="p-3 flex-1 overflow-y-auto space-y-1">
          <Link href={`/c/${companyId}/p/${projectId}/billing`} className="flex items-center gap-2 px-3 py-2 text-xs text-muted hover:text-foreground hover:bg-white/[0.03] rounded-lg transition-all">
            ← Subcontractors
          </Link>
          <div className="pt-2 pb-1 px-3 text-[10px] font-bold text-muted uppercase tracking-wider">Subcon Performance</div>
          {[

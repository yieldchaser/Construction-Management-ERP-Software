  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      ...
      <aside className="w-64 border-r border-border-custom bg-card flex flex-col justify-between h-full shrink-0">
        <div className="flex flex-col overflow-y-auto flex-1">
          ...
          <nav className="p-4 space-y-2">
            <Link
              ...
              className="flex items-center gap-2.5 px-3 py-2 text-xs text-muted hover:text-foreground hover:bg-elevated rounded-lg"
            >
              <span>←</span> Back to Dashboard
            </Link>
            
            <div className="pt-4">
              <span className="text-[10px] uppercase font-bold text-muted tracking-wider block px-3 mb-2">
                Planning Module
              </span>
        ...
        <div className="p-4 border-t border-border-custom bg-black/20 text-[10px] text-muted space-y-2 shrink-0">
          <div className="flex items-center justify-between">
            <span className="font-bold text-muted">TO DO PENDING</span>
          ...
          <div className="flex items-center gap-1.5 opacity-80 mt-1">
            <span>💾</span> Database Backup: <strong className="text-muted">5:40 AM, 29 Aug</strong>
          </div>
        </div>
      </aside>
          ...
          <div className="flex gap-1">
            {(["wbs", "milestones", "baseline", "lookahead"] as const).map(t => (
              <button key={t} onClick={() => setMainTab(t)}
                className={`px-3 py-1.5 text-[10px] font-bold rounded-lg capitalize transition-all ${mainTab === t ? "bg-primary text-white" : "bg-white/5 text-muted hover:text-foreground"}`}>

...
export default function MoMPage() {
  ...
  const loadMoms = async () => {
    ...
    params2.set("project_id", projectId);
  ...
  useEffect(() => {
    loadMoms();
  }, [companyId, projectId, filterStatus, filterType, filterAttendee, filterDate]);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <PageHeader title="Minutes of Meeting" subtitle="Project MOM register and action items">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.status} onChange={(e) => setForm(prev => ({ ...prev, status: e.target.value as typeof MOM_STATUSES[number] }))}
                    className={inputCls}>
                    {MOM_STATUSES.map((s) => (
                      <option key={s} value={s}>{s}</option>
                    ))}

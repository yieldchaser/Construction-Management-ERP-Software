...
BASE = "http://127.0.0.1:8012/apis/v3"
DB_FILE = os.path.join(HERE, "test_phase12.db")
...
def start_server():
    ...
    proc = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "app.main:app",
         "--host", "127.0.0.1", "--port", "8012", "--log-level", "error"],
        env=env,
        cwd=BACKEND_ROOT,
    )
    for _ in range(30):
        time.sleep(1)
        try:
            if requests.get("http://127.0.0.1:8012/").status_code == 200:
                return proc
        except Exception:
            pass
    proc.terminate()
...
def seed_db(company_obj, project_obj):
    ...
    user = models.User(id=uuid.uuid4(), name="Equipment Test User")
    db.add(user)
    db.commit()

...
VERDICTS = {
    "R2-404": ("CONFIRMED", "Fix present on main, and swept to every document. The finding's defect "
               "was that an uploaded logo, signature and stamp were stored and printed nowhere. All "
               "four generators now embed them - watermark behind the pages, letterhead on page 1, "
               "signature band on the last page - and undecodable bytes are SKIPPED honestly rather "
               "than crashing the render or printing a broken image. Same four-document sweep as "
               "R2-607."),
    "R2-546": ("CONFIRMED", "Evidence-close, bounds verified in the settings schema. Decimal-place "
               "settings of 7 and 9 are rejected and unknown grn_numbering and name-display values "
               "are 422 rather than being stored and later failing to match any renderer. A "
               "tripwire test pins it."),
    "R2-264": ("CONFIRMED", "Fix present on main, all three clauses. Subcontractor attendance now "
               "bounds shift_multiplier to 0.5-3.0 (a half shift to a triple shift, so a 100x "
               "multiplier cannot be stored), requires a worker count when overtime or an allowance "
               "is logged, and caps overtime at 12 hours per worker. Same shape as the muster-roll "
               "bounds under R2-263."),
    "R2-296": ("CONFIRMED", "Fix present on main, and closed with an allowlist rather than a "
               "blocklist. The finding's defect was that a work-order amendment could rewrite any "
               "field. Amendments now whitelist exactly {estimated_work_amount, terms}, are applied "
               "in one transaction, and amended_by is stamped from the authenticated user rather "
               "than accepted from the caller - the same actor-identity rule as R2-143 and R2-408."),
    "R2-332": ("CONFIRMED", "Fix present on main, and the key normalisation matches the storage. "
               "The finding's defect was that the subcon attendance idempotency key compared roles "
               "differently from how they were stored, so 'Mason' and 'mason ' produced duplicate "
               "rows. The key now trims and lowercases, and inserts STORE the trimmed role, so the "
               "comparison and the data agree. Same fix shape as the muster-roll upsert under "
               "R2-333."),
    "R2-597": ("CONFIRMED", "Fix present on main, and it fails to zero rather than to a flattering "
               "number. The finding's defect was that a subcontractor with no history scored 100% - "
               "a brand-new vendor presented as a perfect performer. An empty history now yields "
               "0.0 metrics and never a fabricated 100%. Same class as R2-589 (a null is not a "
               "pass) and R2-305 (absent data is not zero waste)."),
    "R2-510": ("CONFIRMED", "Fix present on main, and it is the migration behind the RLS work "
               "already verified system-wide this phase. Tenant predicates plus FORCE ROW LEVEL "
               "SECURITY across 108 tables, with 33 genuinely tenant-less tables explicitly "
               "allowlisted rather than silently skipped, and missing-table coverage in migration "
               "20260824_000001. Consistent with the standing finding that the policies are correct "
               "and deliberately inert because the app connects as the owner role - a founder "
               "decision, not a defect."),
    "R2-517": ("CONFIRMED", "Fix present on main. The finding's defect was that Supabase returns a "
               "RELATIVE signedURL and the code used it as a full URL, producing a broken host. "
               "supabase_storage.py:35 builds the storage base as "
               "SUPABASE_URL.rstrip('/') + '/storage/v1' and create_signed_url (:112) joins the "
               "returned signedURL (:125) onto it, so the link resolves."),
    "R2-267": ("CONFIRMED", "Fix present on main. The finding's defect was that the Tally export "
               "booked settlement bills as Purchase vouchers, so cash movements entered the "
               "customer's books as cost. tally.py now types vouchers through a "
               "Literal['Sales','Purchase','Receipt','Payment','Journal'] (:80) and routes "
               "settlements to Receipt for money-in and Payment for money-out (:237), never "
               "Purchase - the same direction-aware classification as R2-238."),
    "R2-369": ("CONFIRMED", "Fix present on main. The finding's defect was that re-downloading an "
               "export produced different voucher numbers, so the same transactions could enter "
               "Tally twice under different identities. The connection holds last_voucher_seq "
               "(tally.py:201, :226) and numbers derive from it, so a re-download is byte-identical "
               "and only mark-synced consumes the sequence."),
    "R2-409": ("CONFIRMED", "Fix present on main, and the register is honest that the code lives in "
               "hr.py rather than the file the row names. The payslip CSV now carries an identity "
               "block - pay period, run id, company and project - so an exported payslip file can "
               "be attributed to the run that produced it rather than being an anonymous table of "
               "numbers."),
    "R2-542": ("CONFIRMED", "Fix present on main, all three clauses. mark-synced requires "
               "settings:manage rather than bare membership; an unmark-synced route exists so a "
               "mistaken sync can be reversed rather than permanently consuming the vouchers; and "
               "/tally/pending reports pre-window exclusions so the operator can see what the "
               "export is leaving out instead of silently receiving a short file."),
    "R2-595": ("CONFIRMED", "Fix present on main. The finding's defect was that the Tally export "
               "guessed the bank or cash ledger name. It now resolves the ledger from the payment's "
               "OWN account through the bank mapping table (tally.py:221-222, keyed on "
               "onsite_bank_account_details) with _cash_bank_type (:191-194) distinguishing cash "
               "from bank, and /tally/mappings/bank gives that table a writer so the mapping can be "
               "maintained rather than being an unfillable lookup."),
    "R2-258": ("CONFIRMED", "Fix present on main. The finding's defect was that a same-day timesheet "
               "whose end_time was not after its start_time wrapped into a phantom 23-hour shift "
               "rather than being rejected. It is now a 422. Same family as R2-562 (negative "
               "durations impossible) and R2-753's date handling."),
    "R2-241": ("CONFIRMED", "Fix present on main - this is the root change that also closes R2-133, "
               "R2-538 and R2-350. The verdict is server-computed from the variance "
               "(three_way.py:177), match_status and matched_by are removed from the create schema "
               "and documented as server-computed (:28), matched_at is aware UTC (:192, the R2-727 "
               "class), and matched_by is stamped from the session user only when the match "
               "actually matched (:191)."),
    "R2-349": ("CONFIRMED", "Fix present on main, and it removes the caller's ability to state the "
               "amount at all. The finding's defect was that reconciliation compared against a "
               "caller-typed invoice total. three_way.py:25-26 makes invoice_id REQUIRED and "
               "records the rule - the total comes from the identified bill's total_payable, never "
               "typed by the caller - and :106 loads that bill. The console form picks a vendor "
               "bill rather than offering a number field."),
    "R2-539": ("CONFIRMED", "Fix present on main. The finding's defect was that approve and reject "
               "accepted approved_by from the caller, so the approval could be attributed to "
               "anyone. Both now stamp the session user and a server timestamp and IGNORE a "
               "caller-supplied approved_by, and 'rejected' was added to the model's vocabulary so "
               "a rejection is representable rather than being stored as an unmatched string."),
    "R2-594": ("CONFIRMED", "Fix present on main, at both layers. The finding's defect was that the "
               "same PO/GRN pair could be matched repeatedly, so one receipt could authorise "
               "several invoices. three_way.py:120-125 refuses a duplicate pair with 409, and a "
               "unique constraint backs it at the database with a dedupe-aware additive migration - "
               "the live-schema rule. A bill-link amount agreement gate was added in the same "
               "change."),
    "R2-613": ("CONFIRMED", "Fix present on main - filed during the R2-594 closure because legacy "
               "duplicate three-way rows prevented that unique constraint from enabling on "
               "production (the migration skipped itself with a NOTICE while duplicates existed, "
               "which is the correct conservative behaviour rather than failing the deploy). The "
               "purge is keep-earliest and the constraint creation is idempotent, so the migration "
               "converges whether or not duplicates were present. Worth noting for the launch "
               "checklist: the constraint's protection only begins once that purge has actually run "
               "against production data."),
    "R2-383": ("CONFIRMED", "Fix present on main, and solved within the constraint honestly. The "
               "finding's defect was that a recurring to-do never recurred. todos.py:89-101 spawns "
               "the next occurrence AT COMPLETION TIME, with the docstring stating exactly why - no "
               "scheduler exists - rather than pretending a cron does the work. Daily, weekly and "
               "monthly are handled with the month-end clamp, and assignees are copied forward."),
    "R2-442": ("CONFIRMED", "Fix present on main, on both the read and the write. The finding's "
               "defect was that a to-do could hold a javascript: URL rendered as a link. New writes "
               "are allowlisted to http(s) by field validators on both schemas "
               "(todos.py:140-142, :168), and legacy rows holding another scheme SERIALIZE AS NULL "
               "(:39-46) so an existing malicious value cannot reach the DOM, while remaining "
               "clearable via PUT null. Same remedy as R2-257 and R2-466."),
    "R2-229": ("CONFIRMED", "Evidence-close, and the mechanism is the shared one verified "
               "repeatedly this round: the towers Billed figure is built from _active_bills scoped "
               "to REVENUE_INVOICE_TYPES, so expense and settlement vouchers cannot inflate it and "
               "a cancelled bill cannot count. A behaviour test locks it."),
    "R2-330": ("CONFIRMED", "Same change verified under R2-205 and R2-245, seen from the inventory "
               "side. Wastage gates stock availability before writing, decrements the warehouse "
               "balance, and writes a 'used' MaterialTransaction linked back by source_ref_id - so "
               "the ledger row can be traced to the wastage record that caused it."),
    "R2-412": ("CONFIRMED", "Fix present on main, and it refuses rather than improvising. The "
               "finding's defect was that the ZATCA QR was generated for companies with no VAT "
               "registration, filling the tax slots with the Indian GSTIN. Generation now 409s "
               "unless ZATCA is enabled AND a VAT number is stored, and the GSTIN never fills a VAT "
               "slot (zatca.py builds the TLV from vat_number explicitly, :25-37)."),
    "R2-413": ("CONFIRMED", "Fix present on main. The finding's defect was a fabricated single "
               "'Item' line on the ZATCA invoice. zatca.py:60-72 now reads the real description "
               "keys with a documented fallback chain and derives quantity and rate from the line "
               "itself, and unreconciled totals are a 409 rather than an invoice that does not add "
               "up."),
    "R2-188": ("CONFIRMED", "Fix present on main. The finding's defect was that a Zoho push invented "
               "a contact literally named 'Vendor' when the counterparty had no platform user. The "
               "push now resolves the vendor from the linked LibraryParty - the same "
               "userless-counterparty resolution as R2-400 and R2-419 - so the customer's "
               "accounting system receives the real business name."),
    "R2-272": ("CONFIRMED", "Fix present on main, and the register is honest that the code lives in "
               "billing.py rather than the file the row names. The tax-invoice PDF now carries the "
               "elements the finding listed - document title, recipient GSTIN, place of supply, "
               "HSN/SAC column and the IGST split. NOTE: the HSN/SAC CELL is populated only on the "
               "billing path; the quotation-conversion path leaves it empty, which is the separate "
               "R2-747 filed this round."),
    "R2-368": ("CONFIRMED", "Same idempotency change verified under R2-187. bills.zoho_bill_id is "
               "persisted, a re-push short-circuits with 409, and Zoho's duplicate codes 13011 and "
               "3062 are mapped to 409 rather than being retried into a second document."),
    "R2-125": ("CONFIRMED", "Fix present on main (D4), closed together with R2-041 and R2-319 by "
               "520fb87. The row was correctly held as NEEDS-DECISION because adding IGST to "
               "quotations was a schema and tax-model change, not a bug fix. The quotation now "
               "follows the same site-state place-of-supply rule as the invoice - IGST inter-state, "
               "CGST/SGST halves intra-state (crm.py:701-715) - with project linkage optional and "
               "the change forward-only. IMPORTANT: this row is what makes R2-745 a live money "
               "defect rather than a latent one - the quotation now correctly STORES igst_amount, "
               "and convert_quotation_to_invoice drops it."),
    "R2-218": ("CONFIRMED", "Evidence-close: handleCreateBill refetches the bills list on success, "
               "so a newly created bill appears without a manual reload and the screen cannot "
               "disagree with the server about what exists."),
    "R2-537": ("CONFIRMED", "Fix present on main, and it is the correct transactional shape. The "
               "finding's defect was that log_deletion committed on its own and swallowed its "
               "errors, so the audit row could land without the deletion or vice versa. It now "
               "QUEUES the row on the caller's session (delete_logs.py:29) and the caller's commit "
               "persists both together - all-or-nothing - with the redundant try/except: pass "
               "removed at every call site. Sibling of R2-536, which added the actor; between them "
               "the audit trail is both attributed and atomic."),
    "R2-602": ("CONFIRMED", "Fix present on main - filed during the R2-305 closure as an R2-370 "
               "sweep miss, because project_spend did not exclude Cancelled bills while operational "
               "spend did. Every bill load in analytics.py now carries the exclusion: :244 "
               "(project_spend), :323 (month_spend), :522 and :623 (the financial endpoint). A "
               "cancelled bill books cost in none of them."),
    "R2-053": ("REGRESSED", "PARTIAL - the blocking half is genuinely fixed and the UI half is "
               "still open, exactly as the register row discloses. FIXED: the finding's severity "
               "driver was that a payment request could not be created unless an unrelated field "
               "was filled in, because the schema declared details: str (required) against a "
               "nullable model. finance.py:729-731 is now Optional[str] = None with a comment "
               "naming the model it matches, so the 422 is gone. STILL OPEN: transaction/page.tsx "
               "has ONE state variable shipTo (:454) under ONE label 'Ship To (addressing)' (:815) "
               "posted to FOUR destinations - ship_to on the bill branch (:580, correct), notes on "
               "debit (:593) and credit (:606), and details on the payment request (:619) - so a "
               "delivery address types into a payment request's description. Filed R2-763 at LOW: "
               "no amount is misstated and nothing is blocked, but details is read back and "
               "displayed, so the wrong content surfaces to a later reader."),
    "R2-608": ("CONFIRMED", "Fix present on main for the path the row closed. The finding's defect "
               "was generic 'except Exception -> 500' wrappers swallowing user errors, so a bad "
               "upload looked like a server fault. The CSV upload wrapper is narrowed to "
               "UnicodeDecodeError and answers an honest 400. The register discloses that the "
               "sibling wrappers are logged as residual under the R2-076 pattern rather than "
               "claiming a full sweep - which is the honest closure standard."),
    "R2-609": ("REGRESSED", "PARTIAL - the row named FOUR surfaces and one was gated. FIXED: "
               "finance.py:166-168 validates a payment's cost_code against LibraryCostCode for the "
               "company, 422 naming unknowns, atomic. NOT FIXED, counted by LibraryCostCode "
               "references per file: crm.py writes quotation-item cost_code at :681, :765 and :828 "
               "with ZERO LibraryCostCode references; hr.py writes PayrollProfile cost_code at "
               ":1404/:1419 with ZERO; library.py writes LibraryMaterial.cost_code at :451 and its "
               "three LibraryCostCode references are all the cost-code CRUD endpoints (:268, :272, "
               ":289), none validating the material's own code. R2-334's consequence applies "
               "unchanged - reports.py:544-545 emits Cost Code columns and :670/:690 group by the "
               "value, so an invented code is absent from every rollup. crm.py is the costly one, "
               "since quotation items become invoice lines. Filed R2-764."),
    "R2-385": ("CONFIRMED", "Closed correctly as a decision (CD-9) rather than a code change. "
               "TaskTodo and Todo are both live and console-reachable, and merging them would "
               "require choosing a surviving vocabulary (is_completed versus status) with console "
               "and API blast radius. The boundary between them is now documented and both are "
               "kept. Documenting a deliberate duplication is a legitimate closure; silently "
               "leaving it undocumented would not have been."),
    "R2-010": ("CONFIRMED", "Fix present on main (CD-2). The finding's defect was calculator maths "
               "duplicated between console, public site and server, free to drift - and it did "
               "drift, which is what R2-482, R2-519 and R2-611 each measured. There is now one "
               "shared calc module with contract tests. OBSERVATION carried from R2-482, still "
               "true: frontend/src/lib holds TWO byte-identical copies of that module "
               "(calc-shared.ts and calcShared.ts) and only the first is imported - dead code "
               "today, but a correction applied to the wrong twin would silently not ship."),
    "R2-030": ("CONFIRMED", "Fix present on main (D5). The finding's defect was that a BOQ document "
               "could not gain a line item without re-importing the sheet. There is now an inline "
               "BOQ row and POST /boq-documents/{doc_id}/items, which is the same endpoint whose "
               "cost-code gate and _effective_unit_rate parity were verified under R2-334 and "
               "R2-451."),
    "R2-428": ("CONFIRMED", "Evidence-close: the finance CSV import template is header-only since "
               "the demo-data purge, so a user downloading it cannot mistake sample rows for their "
               "own data or import them by accident."),
    "R2-518": ("CONFIRMED", "Evidence-close, and it is the honest behaviour: a geolocation failure "
               "BLOCKS the punch with an alert (the R2-060 fix) rather than falling back to "
               "fabricated coordinates. No invented coordinates remain anywhere. Related but "
               "distinct from R2-750, where the project API cannot store site coordinates at all, "
               "which is still open."),
    "R2-612": ("CONFIRMED", "Fix present on main - filed during the R2-594 closure as an instance of "
               "the success-toast discipline class. The three-way page's approve and reject "
               "handlers now surface the server's detail on a non-2xx and leave state UNTOUCHED on "
               "failure, so the screen never shows an approval the server refused. Same shape as "
               "R2-465."),
    "R2-605": ("CONFIRMED", "Fix present on main, and the row is a good example of an audit "
               "question paying off - it was filed from the R2-242 closure as 'possible dead "
               "rejection path, needs audit look', and reject_po WAS dead: it neither persisted nor "
               "returned a valid response. procurement.py:681-705 now resolves the PO, gates on "
               "procurement:approve, refuses a PO that is cancelled or closed (409) or already "
               "approved or already rejected (400), records the approval action against the "
               "configured rule when one applies, and then actually writes approval_flag = "
               "'rejected' and commits."),
    "R2-319": ("CONFIRMED", "Closed by the same D4 commit as R2-041 and R2-125, completing D-011. "
               "The state-required gate and site-derived IGST split are in reports.py:1095-1102 "
               "with _gst_split applied at the GSTR builders. Verified in the same read as R2-041."),
    "R2-505": ("CONFIRMED", "Evidence-close: the internal 'PHASE 16' eyebrow is gone from the "
               "production screen and a grep for PHASE across frontend/src returns zero, so no "
               "internal build-phase label remains anywhere in the UI. Same class as R2-406 and "
               "R2-480."),
    "R2-721": ("CONFIRMED", "Fix present on main, and it uncovered a bigger blocker than the row "
               "was filed for. The finding's defect was that report_type was unvalidated free text "
               "compared against fixed strings elsewhere, so an unknown type stored silently and "
               "matched no due-date rule. statutory.py:31 now types it "
               "Literal['pf','esi','bocw','tds','pt','it'] with a before-validator normalising case "
               "(:46-49) so comparisons cannot miss on capitalisation, and an unknown type is a 422 "
               "naming the valid values. The BONUS the register records is the more serious half: "
               "StatutoryReport had NO due_date column at all, so every POST /statutory 500'd - the "
               "nullable column was added via the boot schema-sync, which is the safe additive "
               "path."),
    "R2-614": ("CONFIRMED", "Fix present on main - filed during the R2-404 closure as a "
               "tighten-when-touched item, and tightened. Branding upload now validates the actual "
               "BYTES rather than the declared type: settings.py:841-842 checks PNG and JPEG magic "
               "numbers and :847/:869-872 enforce a 5 MB cap, both 422 naming the problem. Content "
               "type from the client is no longer the thing trusted."),
    "R2-165": ("CONFIRMED", "Fix present on main. The finding's defect was that the chat screen "
               "rendered both panes below the md breakpoint, so on a phone the conversation list "
               "and the thread fought for the same width and neither was usable. The layout now "
               "collapses to a single pane below md with the conversation list as a drawer."),
    "R2-203": ("CONFIRMED", "Fix present on main, and fixed honestly rather than by hiding the "
               "fallback. The finding's defect was that the safety caption stated a literal 50,000 "
               "manhours basis regardless of the data. The caption now renders "
               "stats.total_manhours_used from the API (d/safety/page.tsx:381). CHECKED the server "
               "rather than assuming: safety.py:208-216 sums AttendanceLog.hours_worked and uses "
               "the caller-supplied number ONLY when there is no attendance data, returning "
               "manhours_source so the UI appends '(estimated - no attendance data)'. The client "
               "still sends 50000 at :170, but that is the disclosed fallback, not the displayed "
               "basis when real hours exist."),
    "R2-506": ("CONFIRMED", "Same caption and same commit as R2-203, verified in the same read. The "
               "manhours basis renders the API's total_manhours_used with the estimated-source "
               "disclosure, rather than a hardcoded figure."),
    "R2-216": ("CONFIRMED", "Fix present on main. The finding's defect was that a GRN's billed flag "
               "was client-only state that vanished on refresh, and 'mark billed' pretended to "
               "persist. The page now derives isBilled from the SERVER's three-way matches - it "
               "fetches matches alongside the other collections (d/procurement/page.tsx:117, :128) "
               "and sets isBilled from billedGrnIds (:187) - so the flag survives a refresh, and "
               "the mark-billed control reports honestly that no endpoint exists."),
    "R2-224": ("CONFIRMED", "Fix present on main. The finding's defect was that the timesheet POST "
               "appended 'Z' to a wall-clock value, declaring local time to be UTC and shifting "
               "every entry by the offset. The payload now carries the real UTC offset. Same class "
               "as R2-753 (date-only fields shifted by browser timezone), which remains open."),
    "R2-393": ("CONFIRMED", "Closed by the R2-437 change verified earlier this round: the funnel "
               "stage filters now accept the eight statuses crm.py DEFAULT_STATUSES actually ships "
               "alongside the legacy names, cumulative down the funnel with dead ends excluded. The "
               "custom-status mapping (a company that renames its own statuses) is left as a "
               "founder decision rather than guessed, which is the right call - the shipped "
               "vocabulary is reconciled and a company-specific one cannot be inferred."),
    "R2-394": ("CONFIRMED", "Fix present on main. The finding's defect was that Total Spent read "
               "empty because the page called a POST-only route with GET, took a 405, and swallowed "
               "it into an empty result. It now sums expense-type rows from "
               "GET /finance/transactions - a real endpoint with the shared expense "
               "classification - so the figure is computed rather than silently absent."),
    "R2-395": ("CONFIRMED", "Fix present on main, closed by removing the false offer. The finding's "
               "defect was an export menu offering xlsx, which nothing could produce. The entry is "
               "gone - a grep for xlsx over the reports page returns nothing - so only producible "
               "formats are offered. Same remedy as R2-156 and R2-479."),
    "R2-397": ("CONFIRMED", "Fix present on main, both halves. A popup-blocked print now fails "
               "honestly instead of reporting success against a window that never opened, and the "
               "HTML/print export escapes every interpolation site - escapeHtml is defined at "
               "reports/[slug]/page.tsx:11 and applied to headers and cells in both builders "
               "(:701, :707, :752), so report data cannot inject markup into the printed document. "
               "NOTE: this file's CSV builder is the one carrying the separate csvSafeCell gap "
               "filed as R2-755; the HTML path verified here is complete."),
    "R2-422": ("CONFIRMED", "Fix present on main. The finding's defect was a dashboard rendering "
               "fabricated state before any fetch resolved, so invented figures were visible and "
               "indistinguishable from real ones. The fabricated initial state is gone and "
               "collections initialise empty (projects init []), so the screen shows an empty state "
               "until real data arrives."),
    "R2-424": ("CONFIRMED", "Fix present on main. The finding's defect was that the DPR project "
               "filter offered a hardcoded list of projects that did not exist in the tenant. The "
               "options now derive from the rows actually fetched, so the filter can only offer "
               "projects the user has."),
    "R2-425": ("CONFIRMED", "Fix present on main. The finding's defect was invented worker rows, a "
               "PostGIS caption describing capability the product did not have, and a hardcoded "
               "site name. All three are gone, and the screen consumes the REAL geofence columns "
               "instead - the same columns whose absence at the project API is the separate, still "
               "open R2-750."),
    "R2-427": ("CONFIRMED", "Fix present on main. The finding's defect was mock equipment and BOQ "
               "rows rendered when a fetch failed, so an error looked like data. The pages now "
               "catch honestly and render empty states; no mock rows remain."),
    "R2-445": ("CONFIRMED", "Fix present on main, and it is the right distinction for an audit "
               "screen. The finding's defect was that a failed fetch on the delete-logs page "
               "rendered the same empty state as 'no deletions have occurred' - an all-clear that "
               "was actually a network error. A fetch error now renders a NAMED error row, so "
               "'nothing was deleted' and 'we could not find out' are visibly different."),
    "R2-454": ("CONFIRMED", "Fix present on main and shared across all three surfaces. The "
               "finding's defect was that PDF links opened a new tab with no Authorization header, "
               "so the user got a 401 page instead of a document. lib/siteflow now exports "
               "downloadWithAuth (fetch with auth, blob, anchor, non-2xx throws) and it is used by "
               "billing (:697), procurement (:723) and the project BOQ page (:575). One helper, "
               "three call sites, no unauthenticated tabs."),
    "R2-463": ("CONFIRMED", "Fix present on main - the same sweep verified under R2-462, counted "
               "there as 29 redirect stubs all awaiting params with ZERO missing. This row adds the "
               "other two halves: the stubs carry ?project=<id>, and ProjectContext prefers the "
               "route/query id over the stored one, so a stub cannot land the user in a different "
               "project than the URL names."),
    "R2-465": ("CONFIRMED", "Evidence-close and correct: a publish failure alerts and RETURNS before "
               "setDrawings, so the local list is never updated to reflect a publish the server "
               "rejected. The ordering is the whole fix - surfacing the error while still mutating "
               "state would leave the screen disagreeing with the server."),
    "R2-466": ("CONFIRMED", "Fix present on main, and validated at the write path. The finding's "
               "defect was that drawing file URLs accepted any scheme, so javascript: and data: "
               "URLs could be stored and later rendered as links. drawings.py:25 requires "
               "scheme == 'https' with a non-empty netloc, and :87/:103 reject anything that is not "
               "a same-origin path or an https URL on the product's own storage origin, with a "
               "message stating the rule. Same remedy as R2-257 on timesheets. RESIDUAL, disclosed "
               "in the register row: rows seeded by earlier audit rounds still hold malicious URLs "
               "and need ops cleanup - audit debris in the test tenant, belongs on the launch "
               "cleanup list, not a code defect."),
    "R2-483": ("CONFIRMED", "Fix present on main, and fixed by derivation so the controls cannot "
               "disagree. The finding's defect was that wall thickness and leaf count were "
               "independent inputs, so a user could set a combination describing no real wall. "
               "Thickness now derives from leaves x brick width + joints. Same shape as the server "
               "fix under R2-279, where leaves derive from thickness - each surface derives the "
               "dependent value rather than trusting two inputs to agree."),
    "R2-484": ("CONFIRMED", "Fix present on main. The finding's defect was a stirrup cut length "
               "using a 14d hook allowance where the documented arithmetic called for 6d. "
               "calc-shared.ts:149 computes stirLen = (2 * ((sizeA - 80) + (sizeB - 80)) + "
               "6 * stirrupDiaMm) / 1000 - the 6d the finding specified."),
    "R2-485": ("CONFIRMED", "Evidence-close, and the three remedies were verified individually in "
               "this round rather than taken together on trust: the hidden inputs are exposed and "
               "the fabricated city multipliers dropped (R2-161, CITY_MAP grep returns zero), the "
               "inert selector is removed, and the split lines are honest (R2-163, the console "
               "mirrors the server house-cost math with no double-counted contingency)."),
    "R2-494": ("REGRESSED", "PARTIAL, and the fix landed on the wrong field. The register says the "
               "subcon register 'renders honest em-dash (no fabricated 0%/Rs 0)'. The em-dash "
               "reached status only - d/subcon/page.tsx:88, status: wo.status || '-'. The two "
               "fabrications the row names are STILL LITERALS in the same object: progress: '0%' "
               "(:85) and billedValue: 0 (:87), neither read from the API. Both render as if "
               "measured (:261-267): a 0%-wide progress bar with the text '0%', and fmt(0) as Rs 0 "
               "in a money column, on EVERY work order. Same failure as R2-487's party register "
               "reporting Rs 0 payable against real unpaid bills. The data is derivable - "
               "Bill.wo_id exists (models.py:677) - but WOResponse (billing.py:65-77) carries "
               "neither billed_amount nor progress, so the page has nothing to read and the fix "
               "needs a backend field too. Filed R2-762."),
    "R2-502": ("CONFIRMED", "Fix present on main, and fixed the right way round. The finding's "
               "defect was that output progress was clamped, so a batch that produced 200% of plan "
               "displayed as 100% and the overproduction was invisible. The VALUE is now unclamped "
               "(200% shows 200%) while the BAR keeps its visual cap - the number tells the truth "
               "and the bar does not overflow its container."),
    "R2-516": ("CONFIRMED", "Fix present on main for the half that is in the client's gift. The "
               "finding's defect was that a queued offline punch lost its capture time. Queued "
               "punches now persist the ISO capture timestamp. DISCLOSED LIMITATION, stated in the "
               "register rather than hidden: the server stamps its own now() on replay because the "
               "punch endpoint accepts no client timestamp, so a punch queued offline and synced "
               "later still lands at sync time. That is a backend field, not a client bug, and the "
               "loss the finding measured (the time being discarded entirely) is fixed. Pairs with "
               "R2-515, where the queue itself stopped being deleted."),
    "R2-562": ("CONFIRMED", "Evidence-close. Negative shift durations are impossible since the "
               "guard the register names, so the HR screen cannot render a negative duration."),
    "R2-589": ("CONFIRMED", "Fix present on main, and it distinguishes absent from failed - the "
               "distinction the finding was about. A null is_pass now renders 'Not evaluated' "
               "(d/quality/page.tsx:796) rather than being coerced to FAIL, and the summary tiles "
               "EXCLUDE it rather than counting it as a failure. An un-inspected item no longer "
               "damages the quality statistics."),
    "R2-601": ("CONFIRMED", "Fix present on main. The finding's defect was a steel price hardcoded "
               "at Rs 62/kg, so every estimate used a stale rate no user could change. The rate is "
               "now an editable Steel Rate input alongside an Aggregate Rate input, with "
               "zero-guarded memos so a blank rate cannot divide by zero. Same remedy as R2-161's "
               "editable base rate: the user supplies the local price rather than the product "
               "inventing one."),
    "R2-610": ("CONFIRMED", "Fix present on main - filed during the R2-450/R2-451 closures because "
               "the project BOQ page ignored the import response and recomputed rates client-side, "
               "double-counting the composite rates the server had just been fixed to handle. The "
               "page now mirrors _effective_unit_rate (boq/page.tsx:159 - the explicit rate wins, "
               "else supply + installation, the server's composite-wins rule) and consumes the "
               "import response's skipped_count and warnings (:41-42), so skipped rows are visible "
               "to the user."),
    "R2-170": ("CONFIRMED", "Fix present on main. The finding's defect was that attendance, "
               "drawings and reports were absent from the permission taxonomy, so keys referencing "
               "them were unrepresentable. All three are present in permissions.py (:28, :34, :36 "
               "and again at :52-54). The register discloses something worth keeping: these lines "
               "were lost once to a concurrent-write clobber and re-added, which is why the gate is "
               "now a MECHANICAL scan asserting all 57 require_permission keys exist in the "
               "taxonomy rather than a fixed list - a pin that survives the same accident."),
    "R2-254": ("CONFIRMED", "Fix present on main, and the register is honest that the live site was "
               "not where the row said. The finding's defect was that stock could go negative "
               "without the company's negative_stock_lock being consulted. The flag is a real "
               "column (models.py:57, non-null, defaulting false) and is enforced where consumption "
               "actually happens - production usage via procurement.py:1178 and the DPR "
               "material-consumption path at dpr.py:106-111, annotated there with the reason (DPR "
               "consumes stock just like manual usage). A completed batch over stock is a 400 with "
               "zero transactions written."),
    "R2-297": ("CONFIRMED", "Fix present on main. The finding's defect was that a material's unit "
               "could be changed while stock existed, silently reinterpreting every existing "
               "quantity - 500 bags becoming 500 tonnes. The change is now refused with 422 while "
               "on_hand_qty or reserved_qty is non-zero, so the unit can only change when there is "
               "nothing on hand to reinterpret."),
    "R2-372": ("CONFIRMED", "Fix present on main, all three clauses. A PO carrying an indent_id "
               "flips that indent to ordered; a SECOND PO against the same indent is a 422; and "
               "ordering more than the approved indent quantity is a 422 NAMING the material. The "
               "contract is stated at procurement.py:80-81 - quantities validated and capped at the "
               "approved amounts. An approved indent can no longer be drawn against twice."),
    "R2-387": ("CONFIRMED", "Fix present on main, and it keeps the three quantities separable, "
               "which is the point. The finding's defect was that a stock adjustment was "
               "indistinguishable from a receipt or a consumption. An 'adjustment' now requires a "
               "reason (procurement.py:180, :217, mandatory and stored on the row), a zero "
               "adjustment is rejected (:222-224), adjustments are EXCLUDED from received and "
               "consumed (:194) and folded into current_stock separately - so a physical-count "
               "correction cannot masquerade as procurement or usage."),
    "R2-478": ("CONFIRMED", "Fix present on main. The finding's defect was that the po_restriction "
               "setting configured nothing. procurement.py:515-517 now blocks an unlinked PO with "
               "403 when the company has po_restriction set, while the approved-indent path returns "
               "201. The remaining inert settings (bom_restriction and siblings) are left "
               "founder-gated on the R2-479 precedent - offered only where enforced - rather than "
               "guessed at."),
    "R2-488": ("CONFIRMED", "Fix present on main, and it is the same unit discipline as the "
               "analytics reconciliation verified under R2-329. The finding's defect was a stock "
               "figure summing across bags, tonnes and cft into one meaningless scalar. The "
               "material page now renders a PER-UNIT breakdown and never a cross-unit sum, and rows "
               "keep a non-null unit so the grouping key always exists."),
    "R2-607": ("CONFIRMED", "Fix present on main, and swept to all four documents rather than the "
               "one that was filed. Filed during the R2-403 closure because the supplier identity "
               "reached the bill PDF only. resolve_supplier_tax_details is now called by every "
               "document generator: billing.py:730 (bill), procurement.py:1283 (PO), "
               "budgeting.py:658 (BOQ) and reports.py:203 (client report) - the same R2-403 "
               "machinery, four call sites."),
    "R2-021": ("CONFIRMED", "Fix present on main (D1). The finding's defect was that the project "
               "dashboard's in/out figures were mislabelled and the margin was computed on "
               "GST-inclusive amounts, overstating it. The labels are now Billed In and Billed Out "
               "and the margin is computed ex-GST. The place-of-supply machinery the same file "
               "imports (projects.py:333, :400, project_state_code) is the D4 work verified under "
               "R2-041."),
    "R2-421": ("CONFIRMED", "Fix present on main, and shared so the two writers cannot diverge. The "
               "finding's defect was that project status was free text on one write path, so a "
               "status outside the vocabulary could be stored and would then match no filter. "
               "PROJECT_STATUS_PATTERN is derived from CANONICAL_PROJECT_STATUSES in constants.py "
               "(:85) and applied by BOTH writers - projects.py PUT and planning.py PATCH (:820) - "
               "so an off-vocabulary status is a 422 from either direction."),
    "R2-299": ("CONFIRMED", "Fix present on main, and the security-relevant part is done carefully. "
               "The finding's defect was that the public lead endpoint's rate limit was per-process "
               "and keyed on an address the proxy replaced, so it barely limited anything. The "
               "limiter is now fleet-wide when a storage URI is configured (rate_limit.py:54-56, "
               "with a startup warning when it is not) and the bucket key is proxy-aware behind an "
               "explicit trust flag - public_leads.py:74 states that the forwarded header is NEVER "
               "trusted for anything security-critical, which is the correct posture for a header "
               "the client controls. The endpoint carries @limiter.limit('5/hour') at :176, and the "
               "register records a live burst test reaching 429."),
    "R2-545": ("CONFIRMED", "Fix present on main, and the gate is fail-closed and constant-time. "
               "The finding's defect was that captured public leads reached no operator - the rows "
               "existed and nothing could read them. public_leads.py:139-153 adds an operator "
               "listing (newest-first, bounded limit, returning email_sent and created_at so a "
               "delivery failure is visible). The gate refuses when ADMIN_MIGRATION_SECRET is unset "
               "(:124) rather than defaulting open, and compares with hmac.compare_digest (:129) "
               "rather than ==."),
    "R2-041": ("CONFIRMED", "Fix present on main (D4), and this is the row the whole place-of-supply "
               "sweep hangs off. The finding was deferred under D-011 pending schema, then closed "
               "by 520fb87: reports.py:1095-1102 documents the rule - place of supply derives from "
               "Project.state, compared against the supplier GSTIN's first two characters - and "
               "_gst_split is applied at the GSTR builders (:1143, :1185) emitting CGST/SGST/IGST/"
               "UTGST. A missing Project.state is a 422 so no new indeterminate row can be created, "
               "and the change is forward-only with a would-change report rather than a silent "
               "rewrite of history. The one surface this sweep MISSED is the Tally exporter, filed "
               "this round as R2-744."),
    "R2-598": ("CONFIRMED", "Fix present on main. The finding's defect was that an RFQ had no "
               "lifecycle - it could not be sent or closed, so quotes could be submitted forever. "
               "rfq.py now implements draft -> sent -> closed with each transition guarded by a 409 "
               "naming the current status (:280-282 send, :295-296 close), and quote submission is "
               "refused on a closed RFQ (:214-215). The register records this as CD-7 direction (a) "
               "with the founder to confirm the chosen lifecycle - the mechanism is sound either "
               "way."),
    "R2-530": ("CONFIRMED", "Fix present on main. The finding's defect was that PPE compliance "
               "accepted negative worker counts. safety.py:65-66 bounds both at the schema "
               "(total_workers and compliant_workers Field(..., ge=0), annotated - a headcount "
               "cannot be negative and neither can the compliant subset) and :316 adds the "
               "relational check that the compliant subset cannot exceed the total."),
    "R2-390": ("CONFIRMED", "Fix present on main, and the ordering is what makes it correct. The "
               "finding's defect was that assigning a team role COMMITTED the change and then "
               "crashed 500 while serializing the response, because the member's backing user row "
               "was gone - so a privilege change landed while the operator was told it had failed, "
               "and would likely retry. settings.py:667-683 commits, then resolves the user ONCE "
               "and tolerates its absence, returning 200 with the new role and 'Unknown member' as "
               "the name. The state and the report now agree."),
    "R2-301": ("CONFIRMED", "Fix present on main. The finding's defect was that the delete-log row "
               "written when a payment was deleted carried no actor, so the audit trail recorded "
               "the destruction but not who caused it. finance.py:316 now passes "
               "deleted_by=current_user.name, and the payment-request delete at :1503 does the "
               "same. Generalised by R2-536."),
    "R2-536": ("CONFIRMED", "Fix present on main, and verified by my own AST scan rather than "
               "by trusting the register's count. The finding's defect was that log_deletion "
               "call sites omitted the actor. The signature is now "
               "log_deletion(db, company_id, entity_type, entity_id, summary, party_name=None, *, "
               "deleted_by: str) - deleted_by is KEYWORD-ONLY and has no default, so an omission is "
               "a TypeError rather than a silent None. Scanned every call across backend/app: 32 "
               "call sites, ZERO missing deleted_by (register said 30; the extra two are the "
               "settings pair). Self-tested the scanner against a synthetic call with and without "
               "the keyword before believing it."),
    "R2-259": ("CONFIRMED", "Fix present on main, and closed as an append-only ledger rather than "
               "a mutable flag. The finding's defect was that a drawing approval could be flipped, "
               "erasing the record of who approved the sheet site teams were building from. "
               "drawings.py:296-298 states the rule in code - approval is append-only, an approved "
               "revision is terminal and can never be un-approved, superseding requires a NEW "
               "revision - approval_status is pattern-constrained to approved/rejected/pending "
               "(:108), and approved_by is stamped from the caller. R2-365's superseding semantics "
               "ride on the same model (:50-53)."),
    "R2-366": ("CONFIRMED", "Fix present on main. The finding's defect was that a new drawing "
               "revision could reuse an existing revision's file, so 'Upload New Revision' produced "
               "a revision showing the old sheet. drawings.py:248-255 now rejects it: a query for "
               "the same drawing and file_url raises with a message NAMING the revision that "
               "already points at that file and stating the rule (each revision must reference its "
               "own file). The URL scheme and host restrictions are the separate R2-466 fix."),
    "R2-357": ("CONFIRMED", "Fix present on main. The finding's defect was that equipment "
               "deployment recorded no hours, so plant cost in the P&L was inferred from wall-clock "
               "elapsed time - a machine deployed for a month billed 720 hours. "
               "EquipmentDeployment.hours_used is now a real field (equipment.py:55, ge=0) written "
               "on create (:183), and the P&L prefers it - finance.py:574-577 uses dep.hours_used "
               "when present and falls back to the wall-clock derivation only for legacy rows with "
               "none. Recorded hours beat inferred hours."),
    "R2-531": ("CONFIRMED", "Fix present on main, and BOTH halves the finding proved live are "
               "closed. The defect was that equipment availability keyed off a maintenance "
               "record's status and ignored its date - booking a December service took the machine "
               "off the road today, and completing any one job marked it available while other work "
               "was still scheduled. equipment.py:338-355 now derives Equipment.status from the "
               "OPEN schedules' dates against an aware now(): due_open is true only when some "
               "non-completed schedule is dated on or before today, excluding the (service_type, "
               "scheduled day) the current record closes. A future-dated booking no longer grounds "
               "the machine, and completing one job cannot clear the flag while other due work is "
               "open. completed_date is timezone-aware (:327-328), the R2-727 class of bug."),
    "R2-556": ("CONFIRMED", "Fix present on main. The finding's defect was that equipment write "
               "paths used foreign keys without resolving them, so a bad reference produced an "
               "opaque failure rather than a named one. Every referenced entity is now resolved "
               "before use and 404s naming what was missing - company (equipment.py:122-125), "
               "equipment (:158, :207, :229, :310), project (:164, :235), deployment (:204) - "
               "annotated at :122 with the rule."),
    "R2-570": ("CONFIRMED", "Fix present on main, with all three guards the finding asked for, "
               "applied before anything is written (annotated at equipment.py:239). Temporal: the "
               "fuel log's date is bounded. Relational: a covering deployment is required, so fuel "
               "cannot be booked to a machine that was not on the project. Odometer: :259-267 reads "
               "the machine's previous non-null reading and rejects a lower one with a message "
               "quoting that reading, so the meter cannot run backwards."),
    "R2-513": ("CONFIRMED", "Fix present on main. The finding's defect was that legacy "
               "FaceRecognitionLog rows with a NULL created_at made the listing 500. created_at is "
               "now Optional on the response model (face_recognition.py:40) and both orderings use "
               ".nulls_last() (:82, :109), so a null row serializes honestly instead of crashing "
               "the list. CHECKED rather than assumed: the daily-summary loop calls "
               "log.created_at.strftime() unguarded at :117/:121, but its query filters "
               "created_at >= target AND < next_date (:103-104) and a NULL fails both comparisons "
               "in SQL, so no null row reaches that loop. Not a defect."),
    "R2-175": ("CONFIRMED", "Fix present on main. The finding's defect was that uploaded files "
               "could never be deleted. files.py now has a delete path that removes the storage "
               "object, the row and writes a delete-log entry in the same transaction (annotated at "
               ":325), gated on tenant membership and the data:delete permission, and folder delete "
               "refuses with 409 while the folder is non-empty rather than orphaning its contents."),
    "R2-265": ("CONFIRMED", "Fix present on main. The finding's defect was in the asset "
               "depreciation chain - the first entry could record an accumulated figure "
               "inconsistent with the amount being depreciated. assets.py:56-57 bounds both at the "
               "schema (ge=0) and :133-134 computes in Decimal rather than float, with the first "
               "entry required to satisfy accumulated == depreciation_amount. The rate-vs-life and "
               "chain-continuity parts landed in the sibling commits the register names."),
    "R2-179": ("CONFIRMED", "Fix present on main, and this row is the mechanism the R2-178/R2-479 "
               "closures depend on. The finding's defect was that approval feature types were "
               "hand-listed per file and drifted. app/approvals.py now holds the canonical "
               "APPROVAL_FEATURE_TYPES tuple; settings.py imports it (:20) and types the field as "
               "Literal[tuple(APPROVAL_FEATURE_TYPES)] (:196) so an unlisted category cannot be "
               "stored; finance.py and procurement.py import the enforcement constants from the "
               "same module. NOTE: the frontend mirrors the tuple as a hand-written array whose "
               "accompanying PROSE has since gone stale - filed R2-761, which does not affect this "
               "row's verdict."),
    "R2-238": ("CONFIRMED", "Fix present on main, and this row is worth reading as a warning about "
               "re-drift: the register records that the original fix DRIFTED and was caught live "
               "(settlement vouchers booked as Material Bill/Cost with an inverted sign), then "
               "re-fixed at 125ebfa. The live tree carries the re-fix: finance.py:434 classifies "
               "through is_settlement_money_in and :444-452 signs the row by DIRECTION - type "
               "Receipt or Expense, amount positive or negative, debit or credit - rather than "
               "assuming one direction. The shared classifier lives in bill_scope.py (D3) so the "
               "buckets cannot drift per call site again."),
    "R2-311": ("CONFIRMED", "Fix present on main, at both layers the finding named. The rate "
               "limiter is proxy-aware - rate_limit.py:46-48 reads the first hop of "
               "x-forwarded-for rather than the peer address, which on Render is the internal "
               "proxy - and it uses shared storage when configured (:54-56, storage_uri from "
               "RATE_LIMIT_STORAGE_URI) so limits survive across workers instead of living in "
               "per-process memory. Paired with the --forwarded-allow-ips deployment check "
               "verified under R2-337."),
    "R2-328": ("CONFIRMED", "Fix present on main. The finding's defect was that company "
               "transactions were scoped by project, so payments with no project silently vanished "
               "from the list and the cash balance disagreed with it. finance.py now scopes by "
               "company_id (Payment at :226, Bill at :266, BankAccount at :194), so a project-less "
               "payment is included and the balance agrees with the rows shown."),
    "R2-192": ("CONFIRMED", "Fix present on main across the integration routers. The finding's "
               "defect was that OAuth authorize and export endpoints required only company "
               "membership - anyone in the tenant could connect or disconnect the company's "
               "accounting and storage integrations. Now gated: google_sheets.py:196 and "
               "google_drive.py:225/:354 require settings:manage; zoho_books.py:354/:493 the same "
               "(and push_bill requires billing:edit per R2-189); and the payroll export is behind "
               "require_module_view(..., 'payroll') at google_sheets.py:337, annotated with the "
               "reason - it exposes the data the in-app payroll views gate."),
    "R2-606": ("CONFIRMED", "Fix present on main - filed during the R2-381 closure because "
               "run_payroll is a period-based write that the restrict-entry-creation window did not "
               "cover, the same class as the payment writes that were gated. "
               "enforce_entry_creation_window is imported at hr.py:36 and called at :747 against "
               "month_end (the period's closing boundary, which is the right date for a "
               "period-scoped document) and at :1092 for the sibling dated write. A backdated "
               "payroll run into a closed period is now a 400."),
    "R2-263": ("CONFIRMED", "Fix present on main. The finding's defect was that muster-roll figures "
               "were unbounded, so impossible labour could be recorded. labour.py bounds them at "
               "the schema (workers_present and overtime_hours Field(None, ge=0) at :127/:130, "
               "hours bounded by workers_present x 24, overtime by hours_worked) and adds the "
               "relational rule at :268-269: wages recorded against zero workers is a 422 naming "
               "the rule."),
    "R2-333": ("CONFIRMED", "Fix present on main, and treated as the statutory register it is. The "
               "finding's defect was that re-posting the same muster-roll day created duplicate "
               "rows. labour.py:381-389 now upserts on the natural key - project + contractor + "
               "calendar day + role, compared TRIMMED and case-insensitively so 'Mason' and "
               "'mason ' are the same row - and a replay updates in place. The annotation names it "
               "a statutory register, which is why duplication matters."),
    "R2-415": ("CONFIRMED", "Fix present on main. The finding's defect was that the BOCW return was "
               "assembled from unvalidated input rather than derived from the company's own "
               "records. The month is validated, the contractor is resolved from the party store "
               "through the shared resolver (labour.py:26-29, the R2-131 helper, so an external "
               "contractor with no login still names correctly), and the figures derive from "
               "attendance, payroll and the bill ledger - the same ledger base as the BOCW cess "
               "computation verified under R2-128."),
    "R2-507": ("CONFIRMED", "Fix present on main, and it refuses rather than inventing. The "
               "finding's defect was that omitted muster figures defaulted to zero, so an "
               "incomplete post looked like a day with no labour. labour.py:302-320 derives "
               "(present, absent, hours, overtime) for a project-day from the attendance and crew "
               "records, and :247-249 derives workers_count from attendance when the caller omits "
               "it; a day with nothing to derive from is a 422 rather than a fabricated zero."),
    "R2-440": ("CONFIRMED", "Fix present on main, on both paths that create parties. The finding's "
               "defect was that parties created outside the Library screen got no party id, so they "
               "could not be referenced in the register. next_party_id_custom (library.py:131) is "
               "now called from the subcontractor-create path (billing.py:1552) and the CRM "
               "won-lead path (crm.py:61), with the Library's own create defaulting through it at "
               ":156. No path produces an ID-less party."),
    "R2-384": ("CONFIRMED", "Fix present on main, and it fails CLOSED, which is the part that "
               "matters for a notification path. The finding's defect was that assignment "
               "notification recipient resolution silently produced nothing for several data "
               "shapes. Resolution now covers all five shapes and a delivery failure raises rather "
               "than being swallowed (mailer.py:129, :148-156) so the caller decides how to "
               "degrade, while the fan-out itself is best-effort. The error path also scrubs "
               "addresses out of logged exception text (:38-41, :150-155) - it logs the exception "
               "TYPE and a truncated reason, never repr(exc), so SMTP failures cannot leak "
               "recipient addresses into logs."),
    "R2-291": ("CONFIRMED", "Fix present on main, all three parts. The delete-logs router is "
               "mounted at /apis/v3/delete-logs (main.py:680), path ids are UUID-typed so a "
               "malformed id is a 422 rather than a ValueError-500, and main.py:701-703 adds a "
               "catch-all api_route over /apis/v3/{unmatched_path:path} for all five verbs that "
               "raises a clean 404 - so an unknown API path answers 404 instead of falling through "
               "to the SPA or a 500."),
    "R2-055": ("CONFIRMED", "Fix present on main. The finding's defect was eight project-surface "
               "writes that never checked the response, so a failed write looked like a success and "
               "an optimistic status stayed on screen. projects/page.tsx now carries 8 res.ok "
               "checks - matching the 8 writes the finding counted - each surfacing the server's "
               "detail, and the optimistic status reverts on failure."),
    "R2-059": ("CONFIRMED", "Fix present on main, and it surfaces the SERVER's reason rather than a "
               "generic message. The finding's defect was that payment approve, reject and "
               "mark-paid reported nothing on failure. payment-approval/page.tsx:111-125 now "
               "branches on res.ok, parses body.detail and alerts 'Approval failed: <detail>' "
               "falling back to the HTTP status, and catches the network failure separately with "
               "its own message. NOTE: two payment-request controls remain in the silent set "
               "counted by R2-752; this row's three controls are not among them."),
    "R2-087": ("CONFIRMED", "Evidence-close, and the components were verified independently in this "
               "round rather than taken on the register's word: gst_treatment handling is the "
               "conditional-omission fix confirmed under R2-209/R2-392, the Zoho account mapping "
               "and idempotency under R2-187, and finance-page error surfacing under R2-055/R2-059 "
               "and the R2-752 sweep. No change was needed for this row and none is present."),
    "R2-155": ("CONFIRMED", "Fix present on main. The finding's defect was that the Set Value "
               "screen never showed the value already stored, so a user could not tell a blank "
               "field from an unset one. custom-fields/page.tsx now parses the stored values into "
               "state and renders the current value on each field card. Pairs with the typed-column "
               "dispatch verified under R2-158 - the screen reads back what the server actually "
               "stored."),
    "R2-156": ("CONFIRMED", "Fix present on main, and closed by removing the false choices rather "
               "than by adding stubs. The finding's defect was that the field builder offered "
               "entity types nothing consumed. The select is cut to project and invoice - the two "
               "wired types - and the list follows the user-controlled entity-type filter. Same "
               "remedy as R2-178/R2-479: the offered set equals the working set."),
    "R2-160": ("CONFIRMED", "Fix present on main. The finding's defect was that the concrete tab "
               "shared column state with the steel tab, so switching tabs bled one calculator's "
               "columns into the other. The concrete tab now holds its own column state."),
    "R2-161": ("CONFIRMED", "Fix present on main, and it removes fabricated data rather than "
               "relabelling it. The finding's defect was a house-cost calculator whose city "
               "multipliers were invented. The CITY_MAP multipliers are GONE - a whole-frontend "
               "grep for CITY_MAP returns zero - replaced by an editable base-rate input, with wall "
               "length and contingency exposed as inputs rather than hidden constants. The user "
               "supplies the local rate instead of the product inventing one."),
    "R2-163": ("CONFIRMED", "Fix present on main. The finding's defect was that the console "
               "house-cost figure disagreed with the server calculator, because the console "
               "double-counted contingency. The console now mirrors /calculators/house-cost exactly "
               "- pre-contingency headline, no double-count - so the two surfaces agree."),
    "R2-274": ("CONFIRMED", "Fix present on main. The finding's defect was that a BOQ revision "
               "recorded no comparison against the value it replaced, so revision history could not "
               "show what changed. BOQRevision now carries previous_amount (budgeting.py:728) and "
               "_build_revision_response derives delta = revised_amount - previous_amount "
               "(:744-758), returning None for both when there is no prior value rather than "
               "inventing a zero baseline."),
    "R2-275": ("CONFIRMED", "Fix present on main, gated on both write paths and defended on read. "
               "The finding's defect was that milestone_done could exceed milestone_total, giving "
               "progress above 100%. Create rejects it with 400 (budgeting.py:427-428), PATCH "
               "re-checks the COMBINED state after applying either field (:469-470, so raising "
               "done or lowering total is caught), both are Field(ge=0) at :332/:340, and the PDF "
               "clamps legacy rows that predate the gate."),
    "R2-334": ("CONFIRMED", "Fix present on main, on both entry points. The finding's defect was "
               "that a BOQ cost code outside the company's Cost Code Library would never roll up "
               "into any report. The Excel import collects every code it sees and validates the set "
               "in one query, failing atomically with a 400 that NAMES the unknown codes "
               "(budgeting.py:239-250), and single item create does the same per code (:491-497) "
               "with the same remedy in the message. The column was widened 50->100 with an "
               "additive migration."),
    "R2-449": ("CONFIRMED", "Fix present on main. The finding's defect was that BOQ items could be "
               "created but never edited or removed. budgeting.py:566 adds PATCH /boq/{item_id} and "
               ":626 adds DELETE /boq/{item_id} returning 204. The PATCH recomputes amount through "
               "the same _effective_unit_rate helper the import and create paths use (:312), so "
               "editing an item cannot produce a rate the other paths would not, and it repeats the "
               "cost-code library gate."),
    "R2-450": ("CONFIRMED", "Fix present on main, and the failure is surfaced rather than "
               "swallowed. The finding's defect was that a malformed row made the whole Excel "
               "import 500. The import now counts skipped rows and returns skipped_count with "
               "row-numbered warnings (budgeting.py:159, :178, :197, :260); a row raising "
               "ValueError or TypeError is skipped and reported instead of aborting the batch, and "
               "the budgeting page renders the warnings. The user learns which rows did not import."),
    "R2-451": ("CONFIRMED", "Fix present on main, and centralised - which is what stops the "
               "double-count. The finding's defect was that a BOQ line carrying split supply and "
               "installation rates was priced inconsistently across paths, so the same item "
               "double-counted. _effective_unit_rate (budgeting.py:312) is now the single "
               "composite-wins resolver, called from the import (:214), the amount fallback (:97), "
               "item create and PATCH (:325). One rate rule, four call sites."),
    "R2-453": ("CONFIRMED", "Fix present on main. The finding's defect was that a fake or empty "
               "xlsx produced a 500 rather than a usable error. budgeting.py imports BadZipFile "
               "(:5) and InvalidFileException (:10) and catches both at :268, returning a clean "
               "400; zero-row sheets are handled the same way. Real header errors keep their "
               "actionable detail rather than being flattened into the generic message."),
    "R2-279": ("CONFIRMED", "Fix present on main, and the server is the authority the client twin "
               "was later corrected against (R2-611). The finding's defect was that brickwork took "
               "leaves as a caller-supplied number independent of wall thickness, so a 230mm wall "
               "quoted for a single leaf under-ordered by half. leaves is now DERIVED from "
               "thickness over one nominal leaf and the request field is documented as accepted for "
               "compatibility only (calculators.py:163), the derived value is reported back in the "
               "response, non-physical dimensions are rejected before deriving (:168), and a result "
               "outside the 20-35% mortar band is a 422 naming the computed share and the derived "
               "leaf count (:193-200) rather than a confident wrong quantity."),
    "R2-280": ("CONFIRMED", "Fix present on main, fixed in the shared calculation rather than at "
               "the router. The finding's defect was that a paint job whose door and window area "
               "exceeded the wall area returned NEGATIVE quantities. calc_shared.py:424-425 raises "
               "ValueError('Total opening area exceeds the wall area') as soon as paintable_area is "
               "<= 0, and the router converts it to a 422 naming the rule (calculators.py:230). "
               "Nothing downstream can consume a negative area."),
    "R2-281": ("CONFIRMED", "Fix present on main. The finding's defect was that concrete material "
               "quantities were not derived from the mix at all. calc_concrete_backend "
               "(calc_shared.py:246) now computes dry_volume = wet_volume x CONCRETE_DRY_FACTOR x "
               "(1 + wastage_pct/100) and splits it by the grade's TRUE parts - cement_bags = "
               "dry_volume x (c_parts/total) x 1440 / 50, sand and aggregate by their own parts - "
               "so wastage scales the ordered materials and the mix library holds real ratios."),
    "R2-520": ("CONFIRMED", "Same defect and same commit as R2-281 (8798662, one inseparable diff "
               "naming both ids), verified in the same read of calc_shared.py:246-280. Concrete "
               "materials derive from dry volume via the grade's true ratios; wastage_pct scales "
               "the ordered quantities."),
    "R2-519": ("CONFIRMED", "Fix present on main, and it is the client half of R2-281's server "
               "math. The finding's defect was that the client-side concrete factors disagreed with "
               "the server's ratio arithmetic. The corrected constants are in the shared frontend "
               "module (M7.5 4.0->3.41, M20 8.2->8.06, M25 aggregate 0.76->0.77), the same module "
               "whose M5 cement factor was corrected under R2-482. Full engine consolidation "
               "remains CD-2-gated - a founder decision, not an open defect. The duplicate "
               "byte-identical calcShared.ts noted under R2-482 still applies here."),
    "R2-284": ("CONFIRMED", "Fix present on main and on BOTH mounted paths, which is the part worth "
               "checking. The finding's defect was that a peer-to-peer cash transfer accepted the "
               "same account as sender and receiver, creating a self-transfer that moved nothing "
               "but wrote ledger rows. The sender == receiver case is now rejected with 422 across "
               "both /cashbook/p2p and /finance/cashbook/p2p."),
    "R2-141": ("CONFIRMED", "Fix present on main. The finding's defect was that DELETE on a chat "
               "group answered 405 - the group could be created and never removed. "
               "chat.py:317 registers DELETE /groups/{group_id} returning 204, behind "
               "require_group_admin (:323), so the group is archived by an authorised admin rather "
               "than the verb simply not existing."),
    "R2-142": ("CONFIRMED", "Fix present on main, and all three clauses of the finding are "
               "addressed. Member mutations are gated server-side - require_group_admin "
               "(chat.py:44-46) is called by remove-member (:341) and update-role (:363). Role "
               "values are constrained by CHAT_GROUP_ROLE_PATTERN at both the schema (:126) and "
               "the query parameter (:352), so admin/member/viewer is the whole vocabulary. And the "
               "last admin cannot be removed or demoted - :52-59 counts remaining admins and "
               "raises 400 'Cannot remove or demote the last admin of the group', so a group cannot "
               "be orphaned."),
    "R2-143": ("CONFIRMED", "Fix present on main, and fixed at the authoritative layer. The "
               "finding's defect was that create_group trusted a client-supplied created_by. The "
               "server now DISCARDS it - data.pop('created_by', None) at chat.py:148 - stamps the "
               "resolved creator CompanyTeam row (:150) and inserts that creator as an admin member "
               "in the SAME transaction (:153), so a group can never exist without an admin. Same "
               "root cause as R2-270/R2-469 (a users.id sent into a company_team FK)."),
    "R2-471": ("CONFIRMED", "Fix present on main. The finding's defect was that list_groups "
               "returned every group on the project, so a colleague saw groups they were not a "
               "member of. chat.py:160-178 now resolves the caller's identities (user id plus their "
               "CompanyTeam ids), queries ChatGroupMember for the caller's memberships among the "
               "candidate groups, and filters the list to those - groups = [g for g in groups if "
               "g.id in member_group_ids]. Membership, not project access, decides visibility."),
    "R2-359": ("CONFIRMED", "Fix present on main. The finding's defect was that lead source, "
               "category and status were unvalidated free text, so a typo created a phantom bucket "
               "no report could roll up. All three now resolve against the company's own lookup "
               "tables through the shared _resolve_lead_lookup helper (crm.py:307-309), with "
               "unknown values rejected 400. NOTE: this row does NOT cover priority, which is the "
               "fourth field of the same shape and is still unvalidated - filed separately as "
               "R2-759 while verifying R2-438."),
    "R2-360": ("CONFIRMED", "Fix present on main for what THIS row claims. The finding's defect was "
               "that a quotation could not become an invoice, so the figures were re-keyed by hand "
               "in Billing. models.py adds Bill.quotation_id and crm.py:859-863 adds "
               "POST /quotations/{id}/convert-to-invoice, which records the link, carries the "
               "itemised lines into items_json, and refuses a second conversion with 409 naming the "
               "existing invoice (:891-899) plus a separate 409 on invoice-number clash. The "
               "endpoint is permission-gated (billing:edit) and tenant-checked on both the lead and "
               "the party. SEPARATE DEFECT, already filed: the same function computes gst_amount as "
               "cgst + sgst only, DROPPING quot.igst_amount, so an inter-state quotation converts "
               "to a tax invoice recording zero GST with the tax-inclusive figure as taxable value, "
               "and _validate_bill_line_items is never called - R2-745, whose evidence I sharpened "
               "in this read (see the batch-37 addendum: the Project.state guard at :882-886 cites "
               "IGST Act s.12(3) and then never reads the value)."),
    "R2-158": ("CONFIRMED", "Fix present on main, and fixed by dispatching on the declared type "
               "rather than guessing from the value. The finding's defect was that Set Value sent "
               "everything as text, so dates were unqueryable and a number 0 was dropped. "
               "custom_fields.py:140-182 normalizes the untyped incoming value into the physical "
               "column the field's declared field_type names - number to value_number, date to "
               "value_date, multiselect and checkbox to their own handling - and both the insert "
               "and update branches write all typed columns (:195-196, :205-206), so switching a "
               "value's type cannot leave a stale one behind."),
    "R2-180": ("CONFIRMED", "Fix present on main. The finding's defect was that a typo'd field name "
               "was silently dropped behind a 200, because extra='ignore' is the Pydantic default. "
               "Every custom-fields write model now sets model_config = ConfigDict(extra='forbid') "
               "(custom_fields.py:31, :66, :102), each annotated with the rule - an unknown key is "
               "a 422 naming it, not a silent no-op - and the rejection is atomic."),
    "R2-260": ("CONFIRMED", "Fix present on main, all three clauses. Values are validated against "
               "the DECLARED type and written to the matching typed column (the R2-158 dispatch at "
               ":140-182); field_type itself is constrained by CUSTOM_FIELD_TYPE_PATTERN at :37 so "
               "an unknown type cannot be declared in the first place; the target entity must exist "
               "inside the authorised company; and is_required (:38) is enforced on project and "
               "bill create/update with a 422 naming the missing fields."),
    "R2-151": ("CONFIRMED", "Fix present on main. The finding's defect was that four of the eight "
               "numbers the module exists to produce were hardcoded 0.0 - labour and equipment, "
               "committed AND actual - and the UI rendered them as measured zero rows, so a cost "
               "controller read labour spend as nil on a project with a finalized payroll run. "
               "budget.py:121 now computes labour_actual from the payroll ledger (sum of "
               "PayrollLineItem.net_payable joined to PayrollRun for the project) and :124-141 "
               "computes equipment_actual from real data - deployment hours x Equipment.hourly_rate "
               "plus FuelLog totals plus equipment bills - with the R2-727 aware/naive datetime "
               "normalization applied. labour_committed and equipment_committed remain 0.0 at :120 "
               "and :123, which is honest rather than wrong: no document in the schema commits "
               "labour or plant ahead of spend (there is no labour PO), so there is nothing to sum. "
               "OBSERVATION, not filed: labour_actual joins PayrollRun with NO status filter, while "
               "hr.py:757 and statutory.py:144/:360 all require status == 'finalized'. Traced "
               "whether a draft run can persist: hr.py:780-784 creates the run as 'draft' and "
               ":879 sets 'finalized' in the SAME request before the single commit, and "
               "PayrollRun( appears exactly once in the whole backend, so no draft run with line "
               "items ever reaches the database. The divergence is inert today and becomes live the "
               "day a save-draft-payroll feature is added - worth a filter, not worth a row."),
    "R2-152": ("CONFIRMED", "Fix present on main. The finding's defect was that a GET wrote to the "
               "database - reading committed costs for a project with no budget row PERSISTED a "
               "ProjectBudget row as a side effect. budget.py:69-77 now constructs the zero-valued "
               "ProjectBudget in memory only, never adding it to the session, under a comment "
               "stating the rule: a missing budget row is reported as zeros in memory; a read must "
               "never persist anything."),
    "R2-153": ("CONFIRMED", "Fix present on main. The finding's defect was that committed costs "
               "bucketed only some invoice types, so equipment and expense bills fell outside every "
               "head. budget.py:86-101 now scopes the query to the shared EXPENSE_INVOICE_TYPES "
               "tuple and partitions it head-for-head - purchase to material, subcon to subcon, "
               "equipment to equipment - with an explicit other_actual fallthrough (:100-101) that "
               "is summed into total_actual (:150). No canonical expense type can fall outside a "
               "bucket. Same partition shape as the BI feed fix verified under R2-289."),
    "R2-233": ("CONFIRMED", "Fix present on main. The finding's defect was that Budget vs Actual "
               "booked cost from bills the moment they were typed, with no approval or cancellation "
               "gate. budget.py:86-88 now filters Bill.approval_flag == 'approved' on top of the "
               "shared _active_bills scope (which excludes status == 'Cancelled'), annotated with "
               "the rule at :84-85: an unapproved bill must not book cost the moment it is typed. "
               "Both gates, one query."),
    "R2-237": ("CONFIRMED", "The live confirmation of R2-233, closed by the same change and "
               "verified in the same read. The finding measured 83% of a project's spend missing "
               "from Budget vs Actual on a purpose-built empty project - Rs 70,800 of bills of "
               "which only the purchase bill appeared. Both causes are now fixed together: "
               "EXPENSE_INVOICE_TYPES scoping with an other_actual fallthrough means the equipment "
               "and expense bills reach a head (R2-153), and the approved/non-cancelled gate means "
               "they reach it only when they should (R2-233)."),
    "R2-242": ("CONFIRMED", "Fix present on main. The finding's defect was that committed cost "
               "summed purchase orders in EVERY status, so a draft or cancelled PO counted as "
               "committed spend. budget.py:79-83 now restricts to the status whitelist "
               "('sent', 'partial', 'received'), and the SAME whitelist is applied in both branches "
               "of the tower endpoint (:186-189 and :207-210) - the alignment R2-604 landed."),
    "R2-249": ("CONFIRMED", "Fix present on main. The finding's defect was that the tower budget "
               "breakdown derived Committed differently from the main endpoint. Both now use the "
               "same expression - PurchaseOrder filtered to the sent/partial/received whitelist, "
               "summed on total_amount (budget.py:207-211). The honest limitation is stated in code "
               "at :215-218: documents carry no tower_id (CD-5), so per-tower attribution does not "
               "exist and the endpoint returns ONE 'Overall Project' row rather than repeating the "
               "project's figures per tower - the R2-248 multiplication cannot occur."),
    "R2-250": ("CONFIRMED", "Fix present on main, closed by the same alignment. The finding's "
               "defect was that the tower actual figure was not filtered to expense invoice types. "
               "Both tower branches now call _active_bills(db, project_id, EXPENSE_INVOICE_TYPES) "
               "with the approved-only filter (budget.py:191-195 and :203-206), identical to the "
               "main endpoint, so revenue and settlement vouchers cannot land in a tower's actual "
               "spend."),
    "R2-375": ("CONFIRMED", "Duplicate of R2-249, closed by the same commit (57e5a7d) and verified "
               "in the same read of budget.py. Tower Committed derives from status-whitelisted "
               "purchase orders exactly as the no-towers branch does; pinned by "
               "test_r2_249_tower_budget_committed_from_pos.py."),
    "R2-604": ("CONFIRMED", "Fix present on main - this row is the alignment itself, filed during "
               "the R2-249/R2-250/R2-233 closures because the no-towers branch summed ALL PO "
               "statuses and neither budget branch applied the approved/non-cancelled bill gates "
               "the main endpoint had. Both branches now carry both gates: the PO status whitelist "
               "at :186-189 and :207-210, and _active_bills + approval_flag == 'approved' at "
               ":191-195 and :203-206, each annotated with the rule and the row that required it. "
               "Three code paths, one set of gates."),
    "R2-723": ("CONFIRMED", "Fix present on main and centralised, which is what the row asked for. "
               "The finding was that a CANCELLED bill still booked cost across several "
               "aggregations. backend/app/bill_scope.py now holds the single query scope - "
               "_active_bills(db, project_id, invoice_types) filtering project, invoice type and "
               "status != 'Cancelled' - with a module docstring stating why it exists (R2-232 "
               "landed the exclusion in finance.py; this sweeps the remaining routers so it cannot "
               "drift per-router again). budget.py imports it at :15 and uses it at all three sites "
               "(:86, :191, :203); bi_export.py uses it for the equipment actual verified under "
               "R2-289. The module also carries the D3 shared classifier so settlement types cannot "
               "leak into revenue or expense totals."),
    "R2-401": ("CONFIRMED", "Fix present on main, and gated at the write. The finding's defect was "
               "that invoices printed '(No line items)' because line detail lived in a nullable "
               "items_json blob the create path never required - a tax invoice with no description "
               "of what was supplied, which Rule 46 requires per line. "
               "_validate_bill_line_items (billing.py:919-940) now rejects a null or empty blob, a "
               "non-array, and a payload that does not reconcile to subtotal, each with a 422 "
               "naming the problem, and create_bill calls it at :1000 before anything is stored. "
               "The docstring records the exact user-visible consequence it prevents."),
    "R2-403": ("CONFIRMED", "Fix present on main. The finding's defect was that the company's "
               "GSTIN, legal entity name, phone and address were stored in Settings and printed on "
               "no document - and that the one identity field the invoice did print was the trading "
               "name rather than the registered legal name. billing.py:730 now resolves the "
               "registered supplier identity through the shared "
               "resolve_supplier_tax_details(db, bill.company_id, project) helper (imported at :22 "
               "from app/utils/document_pdf.py, with branch masthead precedence), renders it into "
               "the PDF header (:774) and passes it to the template (:867). The PO, BOQ and client "
               "report siblings were filed separately as R2-607 and the same helper serves them - "
               "confirmed at reports.py while verifying R2-758."),
    "R2-480": ("REGRESSED", "PARTIAL - the finding's own three instances are fixed, and the "
               "replacement copy has since gone stale against a later fix. FIXED: the internal "
               "engineering notes are gone - greps for 'Enforcement gap', 'do not yet consult', "
               "'pending a later round', the custom_pdf_template_enabled disclosure and the "
               "'no effect there' claim all return ZERO in settings/page.tsx. NOT CLEAN: the "
               "replacement text at :2194 says approval chains are 'enforced today on Payment "
               "Requests and Purchase Orders' and that 'for the remaining categories, rules are "
               "saved here but are not yet enforced', which contradicts the sibling notice on the "
               "same panel at :2111 ('Only categories with active enforcement are offered today') "
               "and omits Payment Entries - a category that IS enforced (finance.py:675) and is the "
               "one the dropdown opens on, since approvalCat initialises to APPROVAL_CATEGORIES[0]. "
               "R2-480's rewrite was correct when written; the R2-178/R2-479 fixes then changed "
               "what the panel offers and the prose was not re-swept. Filed R2-761."),
    "R2-329": ("CONFIRMED", "Fix present on main, and the unit problem is fixed at the "
               "aggregation rather than papered over. The finding's defect was that material "
               "wastage was ordered-minus-used summed across every material and every unit - bags "
               "of cement, tonnes of steel and litres of diesel into one scalar - so normal "
               "unconsumed stock read as an 80% waste rate. analytics.py:393-412 now builds "
               "ordered_by_material and consumed_by_material keyed by (material_name, unit) with "
               "the unit normalised, and emits a material_reconciliation list carrying per-line "
               "ordered_qty, consumed_qty, variance_qty and an over_consumed boolean. Two numbers "
               "with no common dimension are no longer subtracted."),
    "R2-498": ("CONFIRMED", "Same code path as R2-329 and R2-305, and the specific contradiction "
               "the finding measured is gone. It caught 'MATERIAL WASTAGE 0%' printed directly "
               "above 'Ordered 300 - Consumed 20515' - consumption 68x procurement reported as the "
               "value meaning nothing is wrong, because max(ordered - consumed, 0.0) clamped it. "
               "The clamp is gone: stock_variance_qty is ordered minus consumed UNCLAMPED "
               "(analytics.py:383) under a comment stating over-consumption must stay visible as a "
               "negative variance, and each material line carries an explicit over_consumed flag "
               "(:411). Wastage itself now reads the MaterialWastage table instead of being "
               "inferred."),
    "R2-499": ("CONFIRMED", "Fix present on main. The finding's defect was that the SUBCONTRACTOR "
               "SCORECARD identified its subject as 'Team 331b67d5' - the literal string 'Team ' "
               "plus eight characters of UUID - because _resolve_team_name walked CompanyTeam->User "
               "only, and subcontractors never log in. analytics.py:118-125 now falls through to "
               "the LibraryParty name via team.library_party_id before giving up, which is where "
               "the real business names live. The terminal 'Team {uuid[:8]}' remains at :125 but "
               "is now reached only when a CompanyTeam row has NEITHER a user nor a library party - "
               "no name exists to print - and it shows an id rather than inventing a plausible "
               "name, so it is honest rather than fabricated. Not filed."),
    "R2-603": ("CONFIRMED", "Fix present on main. The finding (filed during the R2-305/R2-499 "
               "closures) was that GET /analytics/.../financial FABRICATED demo chart data - fixed "
               "'Jun 2026' labels and a 1000 expense point - whenever a company had no bills, so an "
               "empty company saw invented spend. The fabrication block is deleted; the comment at "
               "analytics.py:712 records it, and a whole-file grep for the fabricated month label "
               "and value returns nothing but that comment. A company with no bills now gets an "
               "honestly empty chart."),
    "R2-722": ("CONFIRMED", "Fix present on main, and closed more thoroughly than the register "
               "claims. The finding's defect was a known-credential login path: an OTP demo "
               "allowlist and a fixed demo code shipped as DEFAULTS, plus demo-project seeding that "
               "re-ran on every allowlisted login. config.py:44-45 now defaults "
               "OTP_DEMO_ALLOWLIST and OTP_DEMO_CODE to empty strings (and :67 the email twin), so "
               "an unset environment disables the path entirely - demo_allowlist parses to an empty "
               "set (:232), _is_demo_mobile is False for every number (auth.py:191-192), and an "
               "unconfigured server 503s rather than falling back to a demo code (:343-348). The "
               "seeding is not merely guarded: _seed_demo_projects does not exist anywhere in "
               "backend/app any more, and verify_otp routes an allowlisted number through the same "
               "_post_auth path as any other login with no special-cased tenant (:397-400). MINOR "
               "OBSERVATION, not filed: if an operator sets OTP_DEMO_ALLOWLIST but leaves "
               "OTP_DEMO_CODE empty, use_demo_code issues an empty-string code. Both are "
               "founder-owned env vars and the code is hashed at rest, so this is a hardening nit - "
               "a 503 when use_demo_code is true and the code is blank would close it."),
    "R2-289": ("CONFIRMED", "Fix present on main. The finding proved against the LIVE BI feed that "
               "the budget-variance export omitted expense and equipment bills entirely, so it "
               "disagreed with the Budget module on the same project. bi_export.py:258-303 now "
               "partitions the shared EXPENSE_INVOICE_TYPES scope head-for-head the way Budget "
               "does: material, subcon and equipment each get their head, and anything else lands "
               "in a new other_actual bucket (:262, :272) which is summed into total_actual (:303). "
               "Equipment bills feed equipment_actual alongside deployment hours and fuel logs "
               "(:284-301). No canonical expense type falls outside a head."),
    "R2-177": ("REGRESSED", "PARTIAL - the finding named NINETEEN business-record routers with no "
               "delete and no general edit; three record types now have a void path and the row "
               "closed for all nineteen. FIXED, and on the right documents: billing.py:389 "
               "(work order), billing.py:501 (bill) and procurement.py:710 (purchase order) each "
               "expose a cancel endpoint with the double-cancel and linked-record guards the "
               "register describes. NOT FIXED: counted @router.delete across the nineteen named "
               "routers - budgeting 1, files 2, every other zero - and no cancel endpoint either, "
               "so DPRs, inspections, NCRs, safety incidents, equipment records, assets, wastage "
               "entries, RFQs, three-way matches and custom field definitions remain permanent and "
               "uncorrectable. Filed R2-760 at MEDIUM rather than the original HIGH: the money "
               "documents are covered, and I checked the quality/safety/dpr/equipment/assets "
               "console pages - no delete control is offered, so there is no false affordance, only "
               "a missing capability. The ask is either the paths or an explicit D-code plus "
               "BACKLOG line, since neither exists today."),
    "R2-346": ("CONFIRMED", "Fix present on main. The finding's defect was that FIFO settlement "
               "ignored approval entirely - the query filtered on party and status but never on "
               "approval_flag, and every bill starts 'pending', so the product's normal path marked "
               "unapproved bills Paid. finance.py:261 now filters "
               "Bill.approval_flag.in_(SETTLEMENT_APPROVAL_FLAGS), annotated at :41 with the rule "
               "(FIFO settlement may only apply money to bills that have passed review) and at "
               ":256 with the reason it was wrong. Approval is now a precondition of settlement. "
               "Register file column misattributes this to billing.py; the engine is in "
               "finance.py."),
    "R2-350": ("CONFIRMED", "Closed by the change verified under R2-133 and R2-538. The finding's "
               "point was that billing's one hard gate - a bill may only cite an APPROVED "
               "three-way match (_resolve_bill_match_id) - was bypassed by the sibling endpoint "
               "that creates the record it checks, because POST /three-way accepted match_status "
               "from the request body and stored it verbatim. The caller can no longer supply it: "
               "three_way.py:174-177 always computes match_status from the variance against the "
               "tolerance, and :28 records that both match_status and matched_by are "
               "server-computed. The gate's input is no longer caller-controlled."),
    "R2-377": ("CONFIRMED", "Fix present on main, with the schema change the finding said was "
               "missing. The defect was that retention was deducted from every subcon bill and "
               "never modelled as a liability - TransactionDeduction had no released_at, no "
               "release_due_date, no released_amount - so nothing could state what was held or "
               "release it. models.py:697-702 adds all three as nullable columns (annotated, and "
               "nullable so TDS rows are unaffected), the API surfaces them on the deduction "
               "response (billing.py:132-135, :464-466) and "
               "POST /bills/{bill_id}/deductions/{deduction_id}/release (:1333) performs the "
               "release. Outstanding retention is enumerable: released_at IS NULL and type = "
               "Retention."),
    "R2-381": ("CONFIRMED", "Fix present on main, and the money writes specifically are covered - "
               "which was the finding's headline (the window was enforced on three writes and on no "
               "money entry at all). enforce_entry_creation_window is imported at finance.py:31 and "
               "called at :183 (create_payment, on req.payment_date - the date that drives FIFO "
               "settlement order), :1393 (payment request) and :1548 (p2p transfer). The wider "
               "sweep is the one verified under R2-477: 23 calls across nine routers, so the "
               "setting governs the dated writes it claims to govern."),
    "R2-400": ("CONFIRMED", "Fix present on main. The finding proved across four live bills that "
               "the invoice PDF printed 'Party: N/A' whenever the counterparty was not a registered "
               "platform user - the normal case for a vendor - because the name resolved through "
               "CompanyTeam->User only. billing.py:736-753 now tries the linked LibraryParty name "
               "first (:745), falls through to the user name (:748-751), and reaches 'N/A' only "
               "when neither resolves. A userless vendor prints its business name. NOTE: this "
               "site's precedence is the OPPOSITE of the shared resolver's (LibraryParty first "
               "versus user first) - already filed as R2-748; it does not affect this row's "
               "verdict."),
    "R2-039": ("CONFIRMED", "Fix present on main. The finding measured 91 hardcoded-empty columns "
               "across 21 report builders, including Balance Due and Payment Received on a company "
               "sales report. A whole-file grep for the empty-string column literal over "
               "reports.py now returns ZERO, and the named columns (Balance Due, Payment Received, "
               "Net Amount) no longer appear as empty literals. Closed per D-012 by removing the 80 "
               "columns that were never wired rather than by faking values - a report no longer "
               "promises a figure it does not compute."),
    "R2-184": ("REGRESSED", "PARTIAL, and the register's evidence does not support its claim. The "
               "storage half is a legitimate founder deferral - docs/BACKLOG.md:8 records it under "
               "D-010 and states the loss plainly. But the register says the CRITICAL "
               "false-affordance half was 'closed by ab9623e removing 5 upload controls', and "
               "ab9623e touches finance, hr, library and procurement pages ONLY - four pages, none "
               "of them reports, and no change to reports.py. The affordance R2-184 actually named "
               "is intact: d/reports/page.tsx still ships Compile Progress Report (:270), Download "
               "PDF (:226) and a preview iframe (:251), while reports.py still writes the PDF to "
               "REPORTS_DIR on the ephemeral container disk (:21, :207-214), stores "
               "pdf_url=/static/reports/... (:222) and 404s 'PDF file not found on server disk' "
               "once the container recycles (:282-286). Filed R2-758."),
    "R2-611": ("CONFIRMED", "Fix present on main. The finding's defect was that the PUBLIC brick "
               "calculator kept leaves independent of wall thickness - the same under-order bug "
               "R2-279 fixed server-side. CalculatorTools.tsx now derives them the server's way: "
               "leaves = max(1, round(thickness / (bw + mortarJoint))), annotated as the same rule "
               "as the server calculator and stating that a supplied count never overrides it. The "
               "honest geometry band is there too - mortarShare outside 0.20-0.35 is flagged rather "
               "than quoted."),
    "R2-314": ("CONFIRMED", "Fix present on main, and fixed at the key rather than at the name. The "
               "finding's defect was that the party ledger keyed on a display-name STRING, so every "
               "unresolvable counterparty merged into one 'Vendor/Client' bucket and two "
               "same-named parties summed into one line. _build_party_ledger (reports.py:614) now "
               "carries party_key alongside party_name and keys on the stable row id - "
               "party_key = str(obj.party_company_user_id) if set, with the fallback bucket reached "
               "only when there is NO counterparty id at all (:676-678). The docstring states the "
               "rule: names are display-only, same-named parties keep separate balances."),
    "R2-408": ("CONFIRMED", "Fix present on main, and fixed at the authoritative layer. The "
               "finding's defect was that DailyProgressReport.reported_by was free text stored "
               "verbatim, so the author of the primary contemporaneous site record was a raw UUID "
               "on three rows and an arbitrary string on a fourth. dpr.py:122-125 now stamps "
               "reported_by=current_user.name from the authenticated session, under a comment "
               "naming the old behaviour, and the field is gone from the create schema - the caller "
               "cannot supply it."),
    "R2-419": ("CONFIRMED", "Fix present on main. The finding's defect was that the Transaction tab "
               "resolved the counterparty through CompanyTeam->User only, so a LibraryParty with no "
               "platform login rendered as 'Unknown Party' while the Party tab one click away named "
               "it - the largest single liability on the company attributed to nobody. Both "
               "surfaces now route through the shared resolver: finance.py:1144, :1280 and :1343 "
               "call resolve_party_name(db, id, fallback='Unknown Party'), and billing.py:294/:428 "
               "call it for the work-order subcontractor. The fallback is reached only when the id "
               "resolves to nothing."),
    "R2-438": ("REGRESSED", "PARTIAL - two of the finding's three clauses are fixed and the third "
               "is recorded as fixed but is not. FIXED: phone is bounded at the boundary "
               "(crm.py:101 and :135, a digit pattern of 8 to 15 with optional plus), so "
               "'not-a-phone' is a 422 and the UI can no longer prefix +91 onto nonsense; and a "
               "past expected_closure is rejected by a field_validator on BOTH create (:118-128) "
               "and update (:152-165). NOT FIXED: the register claims 'priority vocabulary "
               "normalized' and priority is still unvalidated free text - a bare str defaulting to "
               "medium (:110) and an Optional[str] (:144), written straight through at :326 and "
               ":364, with no Literal, no pattern, no validator and no lower-casing anywhere in the "
               "file. The Medium/medium split the finding measured stands. Filed R2-759."),
    "R2-479": ("CONFIRMED", "Fix present on main, and closed the honest way. The finding's defect "
               "was a Multi Level Approval editor offering FIFTEEN document categories of which no "
               "flow consulted any - an administrator could publish a two-level Rs 5-lakh PO chain "
               "and POs would proceed ungated. The offered set is now exactly the enforced set: "
               "approvals.py:38-42 declares APPROVAL_FEATURE_TYPES as three entries (Payment "
               "Entries, Payment Request, Purchase Order) and :48-51 is a module-level assert that "
               "every enforcement constant is inside it. Both are wired - procurement.py:474, :601, "
               ":653, :694 gate the PO and finance.py:1304, :1358, :1443 gate payment requests, "
               "through the shared find_matching_rule / levels_approved / record_action engine. "
               "Twelve categories that configured nothing can no longer be offered (the R2-178 "
               "half, verified separately)."),
    "R2-294": ("CONFIRMED", "Fix present on main and pinned app-wide. The finding's defect was that "
               "POST /admin/migrations/backfill-rbac was declared TWICE with different "
               "role-assignment policies, so FastAPI ran the first implementation while the "
               "generated schema documented the second - an operator reading the docs would "
               "authorise the wrong migration. admin_migrations.py now declares the path once "
               "(:119). The gate is the general one rather than a pin on this path: "
               "backend/tests/coverage/test_r2_294_no_duplicate_routes.py asserts no duplicate "
               "route across the whole app."),
    "R2-305": ("CONFIRMED", "Fix present on main, and all three compounding problems are addressed. "
               "The finding's defect was that 'material wastage' was computed as ordered minus "
               "consumed (that is unconsumed INVENTORY, not waste), never read the wastage table, "
               "and clamped the result to zero so the contradiction was hidden. analytics.py "
               "now queries MaterialWastage for the project set (:373-379) and reports "
               "recorded_wastage_qty as the wastage figure (:380-382); the ordered-minus-consumed "
               "quantity is kept SEPARATELY as stock_variance_qty (:383) and is NOT clamped, under "
               "a comment stating that over-consumption must stay visible as a negative variance; "
               "and wastage_pct returns None rather than 0.0 when there are no records, so absent "
               "data no longer reads as zero waste."),
    "R2-306": ("CONFIRMED", "Fix present on main. The finding's defect was that the two analytics "
               "endpoints disagreed about the same project's spend by Rs 1,29,800, because the "
               "per-project figure summed EVERY bill - counting a Rs 1,18,000 sales invoice as "
               "spend - while the financial endpoint filtered correctly. analytics.py:244 now "
               "applies the same classification as its sibling: is_expense_invoice_type("
               "bill.invoice_type) and status != 'Cancelled', matching the "
               "EXPENSE_INVOICE_TYPES filter at :522. total_spend and budget_variance derive from "
               "the corrected figure. Pinned by test_analytics_spend_excludes_sales."),
    "R2-726": ("CONFIRMED", "Fix present on main, and fixed so the three sites cannot drift again. "
               "The finding's defect was that the Enterprise Rollup net balance had two INVERTED "
               "terms - it added the liability advance_received and subtracted the asset "
               "to_receive - while the correct expression already existed 150 lines up in the "
               "R2-096 party balance. finance.py:890 now defines a single _net_balance(advance_paid, "
               "to_receive, advance_received, to_pay) helper, called at all THREE sites (:951, "
               ":1103, :1122). One expression, three callers."),
    ...
    "R2-523": ("CONFIRMED", "Fix present on main with a residual the register itself discloses. "
               ...
               "submitted, since UAN is mandatory in an ECR. NOW FILED as R2-756 - initially left "
               "unfiled because the register discloses the residue, but a disclosed residual on "
               "a row marked FIXED is still a live gap. The module remains non-functional for "
               "its stated purpose until the schema lands."),

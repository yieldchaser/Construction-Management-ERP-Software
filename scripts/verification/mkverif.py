...
VERDICTS = {
    ...
    "R2-418": ("UNVERIFIED", "E1 PASS, and the fix is structurally stronger than the finding "
               "asked for. f18ee2b renders each party row's two direction components ("
               "finance/page.tsx:1409 'Pay X / Receive Y'), but more importantly the To Pay / "
               "To Receive tiles are now computed as partySums over the SAME companyParties "
               "array the rows render from (:1273-1282, displayed :1322/:1329) rather than from "
               "the /finance/transactions summary the finding blamed. The two figures therefore "
               "reconcile by construction rather than by agreement - the divergence the finding "
               "measured is no longer expressible. E3 BLOCKED, not skipped: /finance/parties "
               "returns 200 with 0 rows for ZZ R8 Throwaway (no party data) and 403 for AK "
               "Construction, because the JWT is company-scoped (R2-186) to ZZ R8. Needs a "
               "session scoped to a tenant with party rows; see the R3 blocked-on-login list."),

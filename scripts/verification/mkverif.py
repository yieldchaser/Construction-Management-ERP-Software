...
VERDICTS = {
    "R2-588": ("REGRESSED", "THE DEFECT IS LIVE, recorded FIX_VERIFIED against an off-main "
               "commit. The Weekly Timesheet Approvals table is still bound to a state array "
               "nothing populates. Verified EXHAUSTIVELY rather than by sampling: an untruncated "
               "grep for setTimesheets across d/hr/page.tsx returns exactly TWO lines - the "
               "useState declaration (:166) and an optimistic prev.map inside "
               "handleTimesheetAction (:581). No fetch assigns it, so :581 maps over a "
               "permanently empty array and is a no-op. The nearby fetch that looks like the fix "
               "is not: fetchTimesheetLogs (:194-204) calls /hr/timesheets/project/{pid} and "
               "assigns setTimesheetLogs - a DIFFERENT state feeding the separate Daily Activity "
               "table - which is exactly the confusion the finding described. The tab-gated "
               "effect at :435-440 calls that same wrong function. The other /hr/timesheets call "
               "(:624) is a POST that only reads back tsData.id. Backend shape confirms the "
               "distinction: /timesheets/project/{id} returns TimesheetEntry rows (hr.py:515), "
               "not Timesheet headers carrying the draft/submitted/approved status the approvals "
               "table branches on; a header list does exist at /timesheets/company/{id} "
               "(hr.py:537) and is never called. No submit or approve action can be reached "
               "through the console. No new finding number: the defect is R2-588 and needs its "
               "STATUS corrected."),
    ...
    "R2-474": ("CONFIRMED", "Off-main row, fix present on main, and it closes BOTH halves the "
               "finding named. (1) The client no longer asserts its own verification: the "
               "'Simulate GPS lock (On-Site)' control and its isGpsSimulatedVerified state are "
               "gone from the attendance page entirely (grep returns nothing), and the punch "
               "reads real coordinates via navigator.geolocation.getCurrentPosition "
               "(attendance/page.tsx:415-420). (2) The server measures rather than accepts: "
               "hr.py:334-343 resolves the site coordinates and the project's "
               "attendance_radius_meters (default 500), computes haversine_distance_m (:64), sets "
               "within_geofence = distance <= radius, and writes location_verified=within_geofence "
               "with status 'Present (Off-Site)' when outside (:363-371). The request's own "
               "location_verified field is ignored. The log table renders log.location_verified "
               "from the server response (:783), not a client flag. A punch from London "
               "coordinates would now be stored off-site."),
